interface GameData {
    gameSettings: {
        ...
        player: {
            ...
            maxJumps: number; // NEW: Maximum number of jumps allowed
            runAnimationSpeed: number; // NEW: Time in seconds for one full run animation cycle (e.g., 2 frames)
        };
        obstacle: {
            width: number;
            height: number;
...
class Player extends GameObject {
    ...
    private currentRunFrame: number = 0; // Current frame index for running animation (0 or 1)
    private runAnimationTimer: number = 0; // Timer to control run animation frame switching
    ...
    private canJumpInput: boolean = true; // Flag to track if jump input is "fresh"
    private jumpsRemaining: number;        // How many jumps are left
    private maxJumps: number;              // Total allowed jumps
    ...
    constructor(
        ...
        width: number,
        height: number,
        imageRun1: HTMLImageElement, // Now specifically the first run frame
        ...
        super(x, y, width, height, imageRun1); // Default to first run image
        this.originalY = y;
    ...
    update(deltaTime: number, gameSpeed: number) {
        ...
        // Handle slide input (only if not jumping)
        ...
        if (this.y >= this.originalY) {
            ...
            this.runAnimationTimer = 0; // Reset animation timer on landing
        } else {
            this.isJumping = true;
        }
        ...
        if (!this.isJumping && !this.isSliding) {
            ...
            if (this.runAnimationTimer >= this.gameSettings.player.runAnimationSpeed / 2) { // Half the total cycle time for each frame
                this.runAnimationTimer = 0;
                this.currentRunFrame = (this.currentRunFrame + 1) % 2; // Toggle between 0 and 1
            }

            let currentRunImage: HTMLImageElement;
...
class Game {
    ...
    private resetGame() {
        ...
        const playerImageRun1 = this.assetManager.getImage('cookie_run_1')!; // Now specifically 'cookie_run_1'
        ...
        this.player = new Player(
            ...
            gs.player.width,
            gs.player.height,
            playerImageRun1, // Pass the first run frame
            playerImageJump,
            playerImageSlide,
            gs.player.maxHealth,
