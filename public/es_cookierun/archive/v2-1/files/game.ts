...
class Player extends GameObject {
    ...
    private canJumpInput: boolean = true; // NEW: Flag to track if jump input is "fresh"
    private jumpsRemaining: number;        // NEW: How many jumps are left
    private maxJumps: number;              // NEW: Total allowed jumps
    ...
    constructor(
        ...
        this.maxJumps = this.gameSettings.player.maxJumps; // NEW: Initialize maxJumps from settings
        this.jumpsRemaining = this.maxJumps;               // NEW: Start with all jumps available
    }
    ...
    update(deltaTime: number, gameSpeed: number) {
        ...
        if (jumpKeyPressed && this.canJumpInput && !this.isSliding) {
            if (this.jumpsRemaining > 0) {
                ...
                this.jumpsRemaining--; // Decrement remaining jumps
                this.canJumpInput = false; // Mark input as consumed for this press
            }
        } else if (!jumpKeyPressed) {
            // Reset canJumpInput when jump key is released
        ...
        if (this.y >= this.originalY) {
            ...
            this.isJumping = true; // Player is airborne
        }
    ...
    jump(force: number) {
        // We no longer check isJumping here, as jumpsRemaining handles the logic
        // This method just applies the jump force
        this.isJumping = true;
