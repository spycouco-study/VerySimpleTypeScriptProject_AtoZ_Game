...
class TetrisGame {
    ...
    private newPiece(): void {
        ...
        if (this.currentPiece) {
            ...
            if (!this.isValidMove(this.currentPiece, 0, 0)) { // Check collision at spawn
                this.gameOver();
                // After calling gameOver, the game state is GameState.GameOver.
                // Clear current and next pieces immediately after game over
                // to prevent them from being rendered on the game over screen.
                this.currentPiece = null;
                this.nextPiece = null;
                return; // Stop further processing for this newPiece call
            }
        }
        this.canSwapHold = true;
    }
    ...
    private lockPiece(): void {
        ...
        this.clearLines();
        // Only request a new piece if the game is still playing
        if (this.gameState === GameState.Playing) {
            this.newPiece();
        }
    }
