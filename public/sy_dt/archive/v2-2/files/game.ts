...
class TetrisGame {
    ...
    private drawPiece(piece: Tetromino, offsetX: number, offsetY: number, alpha: number = 1): void {
        for (let row = 0; row < piece.shape.length; row++) {
            for (let col = 0; col < piece.shape[row].length; col++) {
                if (piece.shape[row][col] !== 0) {
                    // piece.x and piece.y are the base coordinates
                    // offsetX and offsetY are additional render offsets
    ...
    private drawUI(): void {
        ...
        if (this.holdPiece) {
            // Center the held piece in the panel - piece.x, piece.y will be 0,0 for holdPiece for display
            // Calculate offsets to center it visually
            ...
            const renderOffsetY = Math.floor((50 + (settings.panelHeight / 2 - (pieceShapeHeight * this.blockSize) / 2)) / this.blockSize) - 1; // Adjust Y for label
