...
class TetrisGame {
        ...
        for (let row = 0; row < piece.shape.length; row++) {
            for (let col = 0; col < piece.shape[row].length; col++) {
                if (piece.shape[row][col] !== 0) {
                    ...
                    if (newY < 0) continue; // Allow pieces to be above the board
    ...
    private isValidMove(piece: Tetromino, offsetX: number, offsetY: number, targetY: number = piece.y): boolean {
        // Create a temporary piece for collision check
        const tempPiece = piece.clone();
        tempPiece.x += offsetX;
        tempPiece.y = targetY + offsetY;

        return !this.checkCollision(tempPiece, 0, 0); // Check collision from the tempPiece's new position
    }
    ...
    private drawUI(): void {
        ...
        if (this.holdPiece) {
            // Center the held piece in the panel
