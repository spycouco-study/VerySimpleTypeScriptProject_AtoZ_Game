...
interface GameConfig {
    ...
    reelingUpDurationSeconds: number; // Duration for fish reeling up animation
    assets: {
...
enum GameState {
    ...
    WAITING_FOR_BITE,
    REELING_MINIGAME,
    REELING_UP_ANIMATION, // New state for fish reeling animation
    GAME_OVER,
}
...
class Fish {
    ...
    draw(ctx: CanvasRenderingContext2D, assetManager: AssetManager) {
        ...
        if (img) {
            ...
            // Flip image if moving right (assuming default image faces left)
            if (this.direction === 1) { 
                ctx.scale(-1, 1);
            }
...
class Game {
    ...
    private reelingUpAnimationTimer: number = 0;
    private reelingUpDuration: number = 0;
    private reelingUpTargetY: number = 0;
    private initialBobberYForReelUp: number = 0;
    ...
    async start(): Promise<void> {
        ...
        // Only initialize game components if configuration and assets loaded successfully
    ...
    private initGame(): void {
        ...
        // Reset reeling animation variables
    ...
    private update(deltaTime: number): void {
        ...
        switch (this.currentState) {
            ...
            case GameState.WAITING_FOR_BITE:
                ...
                // Update fish movement, excluding any fish currently under the bobber for a bite
            ...
            case GameState.REELING_MINIGAME:
                ...
                // Update other fish movement during mini-game
                this.fishes.forEach(fish => {
                ...
                break;
            case GameState.REELING_UP_ANIMATION:
                if (!this.currentFishInMinigame) {
                    this.currentState = GameState.WAITING_FOR_BITE;
                    // Reset bobber Y to initial water surface level
                ...
                // Animate bobber moving up
                this.bobber.y = this.initialBobberYForReelUp + (this.reelingUpTargetY - this.initialBobberYForReelUp) * progress;

                // Fish follows bobber's X, and stays slightly below bobber's Y
                this.currentFishInMinigame.x = this.bobber.x;
                this.currentFishInMinigame.y = this.bobber.y + (this.bobber.height / 2) + (this.currentFishInMinigame.height / 2) - 10; // 10px offset for visual attachment
                ...
                if (progress >= 1) {
                    ...
                    // Remove the successfully caught fish from the active fishes array
                    ...
                    this.currentFishInMinigame = undefined;
                    this.currentState = GameState.WAITING_FOR_BITE;
                    // Reset bobber Y to initial water surface level for next cast
                    this.bobber.y = this.config.initialBobberY * this.canvas.height;
                }

                // Update other fish movement during reeling up animation
                this.fishes.forEach(fish => {
                    if (fish !== this.currentFishInMinigame) {
                        fish.update(deltaTime * this.config.fishSwimSpeedMultiplier, this.canvas.width, this.bobber.x);
                    }
    ...
    private spawnFish(): void {
        ...
        if (minSpawnY < maxSpawnY) {
            ...
            if (this.fishes.length > this.config.maxFishOnScreen) {
                // Remove the oldest fish, but only if it's not the one currently engaged in minigame/animation
                let removed = false;
                ...
                // If the oldest one was the current fish, just don't remove anything, or force removal if array gets too big.
                // For now, simpler: if it's the current fish, it won't be in the active array anyway after minigame start.
                // Re-evaluate: currentFishInMinigame is NOT removed from `fishes` array at `initMiniGame`.
                // So we need to ensure we don't remove it from the start of the array if it's the oldest.
                if (!removed && this.fishes.length > this.config.maxFishOnScreen && this.fishes[0] !== this.currentFishInMinigame && this.fishes[0] !== this.fishUnderBobber) {
                    this.fishes.shift(); // Fallback to removing the absolute oldest if not engaged
                }
            }
        } else {
    ...
    private initMiniGame(fish: Fish): void {
        ...
        this.currentFishInMinigame = fish; // Store the actual fish instance (it remains in the fishes array)
    ...
    private resolveMiniGame(forcedOutcome?: boolean): void {
        ...
        if (caught && this.currentFishInMinigame) {
            // Initiate reeling up animation instead of instantly adding score
            ...
            const boatY = waterLineY - boatHeight; // Top Y of the boat image
            
            // Target the bobber to the point where the fishing line originates on the boat
            this.reelingUpTargetY = boatY + this.config.fishingLineOffsetY;

            this.reelingUpDuration = this.config.reelingUpDurationSeconds;
            this.reelingUpAnimationTimer = 0;
            this.currentState = GameState.REELING_UP_ANIMATION;
            ...
            // Remove the lost fish from the `fishes` array
            if (this.currentFishInMinigame) {
                const index = this.fishes.indexOf(this.currentFishInMinigame);
                if (index > -1) {
                    this.fishes.splice(index, 1);
    ...
    private draw(): void {
        ...
        switch (this.currentState) {
            ...
            case GameState.WAITING_FOR_BITE:
            case GameState.REELING_MINIGAME:
            case GameState.REELING_UP_ANIMATION: // Draw gameplay during reeling animation
                this.drawGameplay();
    ...
    private drawLoadingScreen(): void {
        ...
        // Use a fallback string if config.ui.loading is not yet available
    ...
    private drawGameplay(): void {
        ...
        if (boat) {
            ...
            this.ctx.beginPath();
            // Line starts from the boat directly above the bobber's current X position
            // and at a specified Y offset on the boat.
            this.ctx.moveTo(this.bobber.x, boatY + this.config.fishingLineOffsetY);
            this.ctx.lineTo(this.bobber.x, this.bobber.y); // Line goes from line start on boat to bobber
            this.ctx.stroke();
        }

        this.bobber.draw(this.ctx, this.assetManager);
        this.fishes.forEach(fish => fish.draw(this.ctx, this.assetManager)); // All fish, including currentFishInMinigame if present
