...
interface GameConfig {
    ...
    initialBobberY: number; // Ratio of canvas height
...
class Game {
    ...
    private currentFishInMinigame: FishData | undefined; // The fish currently being reeled
        ...
        if (event.code === 'Space') {
            ...
            switch (this.currentState) {
                ...
                case GameState.WAITING_FOR_BITE:
                    // Input is not typically processed here, waiting for fish bite is automatic
                    break;
    ...
    private update(deltaTime: number): void {
        ...
        switch (this.currentState) {
            ...
            case GameState.WAITING_FOR_BITE:
                ...
                if (this.caughtFishName === null) {
                    const fishNearBobber = this.fishes.filter(fish =>
                        Math.abs(fish.x - this.bobber.x) < fish.width / 2 + this.bobber.width / 2 &&
                        Math.abs(fish.y - this.bobber.y) < fish.height / 2 + this.bobber.height / 2
                    );

                    if (fishNearBobber.length > 0 && Math.random() < deltaTime * 0.5) { // 50% chance per second for a bite
                        this.currentState = GameState.REELING_MINIGAME;
                        this.assetManager.playSound('bite');
                        this.initMiniGame(fishNearBobber[0].data); // Use the first fish that bit
                    }
                }
                break;
            case GameState.REELING_MINIGAME:
    ...
    private spawnFish(): void {
        ...
        // Corrected: Fish should spawn around the bobber's Y position to allow for vertical interaction
        const spawnY = this.bobber.y + (Math.random() * 100 - 50); // +/- 50px from bobber Y
     ...
     */
    private initMiniGame(fishData: FishData): void {
        this.miniGameSuccess = 0;
        this.miniGameFailure = 0;
        ...
        this.config.miniGameBasePointerSpeed = 1.0 + (fishData.miniGameDifficulty * 0.5);
    ...
    private resolveMiniGame(forcedOutcome?: boolean): void {
        ...
        if (caught && this.currentFishInMinigame) {
            ...
            // Remove the specific fish that was caught from the list of active fish
    ...
    private drawGameplay(): void {
        ...
        if (boat) {
            this.ctx.drawImage(boat,
                this.canvas.width / 2 - boat.width / 2,
                this.config.initialBobberY * this.canvas.height - boat.height + 20, // Position boat slightly above the water line
                boat.width,
                boat.height
            );
        }
