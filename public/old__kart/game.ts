...
/**
 * 게임 트랙의 구성을 정의하는 인터페이스.
 * `width`: 트랙의 두께
 * `path`: 트랙 경로를 구성하는 점들의 배열 (트랙의 중앙선)
 * `startAngle`: 플레이어 카트의 시작 방향 (라디안)
 * `finishLine`: 골인 지점의 좌표 (두 점으로 이루어진 선분)
 * `midpointLine`: 랩 인정 확인을 위한 중간 지점의 좌표 (두 점으로 이루어진 선분)
 */
interface TrackConfig {
    ...
    midpointLine: { x1: number; y1: number; x2: number; y2: number; }; // 추가된 중간 지점 선분
}
...
function segmentsIntersect(
    p1x: number, p1y: number, p2x: number, p2y: number, // Segment 1
    p3x: number, p3y: number, p4x: number, p4y: number  // Segment 2
): boolean {
...
function distToSegment(px: number, py: number, x1: number, y1: number, x2: number, y2: number): number {
    ...
    if (l2 === 0) return Math.sqrt((px - x1) * (px - x1) + (py - y1) * (py - y1)); // 선분이 점인 경우
...
class Kart {
    x: number;
    y: number;
    prevX: number; // 이전 프레임의 X 위치
    prevY: number; // 이전 프레임의 Y 위치
    angle: number; // 현재 방향 (라디안)
    speed: number; // 현재 속도 (픽셀/초)
...
class Game {
    ...
    private lastTime: DOMHighResTimeStamp = 0; // 마지막 프레임 시간
    private trackConfig: TrackConfig; // 트랙 설정 데이터

    private currentLap: number = 0; // 현재 진행 중인 랩
    private totalLaps: number = 3; // 총 랩 수: 사용자 요청에 따라 3바퀴로 설정됩니다.
    private isGameOver: boolean = false; // 게임 종료 여부
    private lapCooldown: number = 2000; // 랩 중복 카운팅 방지를 위한 쿨다운 (ms)

    // 랩 인정 강화를 위한 상태 변수
    private hasCrossedFinishLineOnce: boolean = false; // 랩 시작 시 결승선을 한 번 통과했는지
    private hasCrossedMidpoint: boolean = false;       // 결승선 통과 후 중간 지점을 통과했는지
    private lastLapCompletedTime: number = 0;          // 마지막으로 랩이 인정된 시간

    constructor(canvasId: string, trackData: TrackConfig) {
        ...
        this.playerKart = new Kart(
            this.trackConfig.path[0].x,
            this.trackConfig.path[0].y,
            this.trackConfig.startAngle
        );
    ...
    private handleKeyDown(event: KeyboardEvent) {
        if (this.isGameOver) return; // 게임 오버시 입력 무시
     ...
     */
    private isPointOnTrack(px: number, py: number): boolean {
    ...
    private checkLapCompletion() {
        ...
        const crossedFinishLine = segmentsIntersect(
            kart.prevX, kart.prevY, kart.x, kart.y,
            finishLine.x1, finishLine.y1, finishLine.x2, finishLine.y2
        );

        const crossedMidpoint = segmentsIntersect(
            kart.prevX, kart.prevY, kart.x, kart.y,
            midpointLine.x1, midpointLine.y1, midpointLine.x2, midpointLine.y2
        );
...
// `data.json`에서 로드될 `TrackConfig` 객체를 전역적으로 선언합니다.
// 실제 환경에서는 JSON 파일을 비동기로 로드해야 합니다.
declare const trackData: TrackConfig;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const defaultTrackData: TrackConfig = {
        width: 100, // 트랙 폭
        path: [ // "&" 모양 트랙 (figure-8)
            {"x": 300, "y": 100},
            {"x": 500, "y": 100},
