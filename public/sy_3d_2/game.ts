...
let bounceSound: THREE.Audio;

let gameConfig: any;
let textures: Map<string, THREE.Texture> = new Map();
let audioBuffers: Map<string, AudioBuffer> = new Map();

let isGameStarted = false;
let keyState: { [key: string]: boolean } = {};
let titleScreenElement: HTMLDivElement | null = null;
...
async function loadAssets(assets: any) {
    const textureLoader = new THREE.TextureLoader();
    const audioLoader = new THREE.AudioLoader();

    const imagePromises = assets.images.map((img: any) => {
        return new Promise<void>((resolve, reject) => {
            textureLoader.load(img.path,
                (texture: THREE.Texture) => { // Explicit type for 'texture'
                    textures.set(img.name, texture);
                    resolve();
                },
                undefined, // Progress callback not used for simplicity
                (error: ErrorEvent) => { // Explicit type for 'error'
    ...
    const soundPromises = assets.sounds.map((snd: any) => {
        return new Promise<void>((resolve, reject) => {
            audioLoader.load(snd.path,
                (buffer: AudioBuffer) => { // Explicit type for 'buffer'
...
function showTitleScreen() {
    ...
    instructionText.textContent = gameConfig.gameSettings.titleText || "Click to Start";
...
function createBall(material: CANNON.Material) {
    ...
    ballBody.addEventListener('collide', (event: { contact: CANNON.ContactEquation }) => { // Explicit type for 'event'
...
function createGround(material: CANNON.Material) {
...
function setupSounds() {
    ...
    if (bgmBuffer) {
        ...
        bgmSound.setVolume(gameConfig.assets.sounds.find((s: any) => s.name === 'bgm')?.volume || 0.3);
        bgmSound.play();
    }
    ...
    if (bounceBuffer) {
        ...
        bounceSound.setVolume(gameConfig.assets.sounds.find((s: any) => s.name === 'bounce')?.volume || 0.8);
...
function animate() {
    ...
    if (!isGameStarted) return; // Only animate if game has started
    ...
    ballMesh.position.copy(ballBody.position as THREE.Vector3);
    ballMesh.quaternion.copy(ballBody.quaternion as THREE.Quaternion);
...
function handleBallInput() {
    ...
    if (keyState['Space']) {
        ...
        if (ballBody.position.y <= threshold && Math.abs(ballBody.velocity.y) < 0.2) {
    ...
    if (keyState['ArrowUp'] || keyState['w'] || keyState['W']) { // Roll forward (along -Z global)
        forceVector.z -= rollForce;
    }
    if (keyState['ArrowDown'] || keyState['s'] || keyState['S']) { // Roll backward (along +Z global)
        forceVector.z += rollForce;
    }
    if (keyState['ArrowLeft'] || keyState['a'] || keyState['A']) { // Roll left (along -X global)
        forceVector.x -= rollForce;
    }
    if (keyState['ArrowRight'] || keyState['d'] || keyState['D']) { // Roll right (along +X global)
        forceVector.x += rollForce;
    }
