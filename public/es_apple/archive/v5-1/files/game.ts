interface GameData {
    ...
    instructionText: string; // 추가: 게임 조작 설명
    instructionProceedText: string; // 추가: 게임 조작 설명 이후 진행 안내
...
class Game {
        ...
        switch (this.state) {
            ...
            case GameState.TITLE:
                if (this.input.isMouseClicked() || this.input.isKeyDown('Space')) {
                    this.state = GameState.INSTRUCTIONS; // 타이틀에서 사용자 입력 시 조작 설명 화면으로 전환
                    this.input.reset(); // 입력 초기화
                }
                break;
            case GameState.INSTRUCTIONS: // 조작 설명 화면 업데이트 로직
                if (this.input.isMouseClicked() || this.input.isKeyDown('Space')) {
                    this.initializeGame(); // 게임 시작 전 초기화
                    this.state = GameState.PLAYING; // 조작 설명 후 게임 플레이 상태로 전환
                    this.input.reset(); // 입력 초기화
                }
                break;
    ...
    private draw(): void {
        ...
        switch (this.state) {
            ...
            case GameState.TITLE:
                ...
                break;
            case GameState.INSTRUCTIONS: // 조작 설명 화면 그리기
    ...
    private drawInstructionScreen(): void {
        this.ctx.fillStyle = 'white';
...
document.addEventListener('DOMContentLoaded', () => {
    // HTML body에 게임 화면 중앙 정렬을 위한 스타일 적용
    document.body.style.margin = '0';
    document.body.style.backgroundColor = '#1a1a1a'; // 캔버스 배경과 대비되는 색상으로 설정
    document.body.style.display = 'flex';
    document.body.style.justifyContent = 'center'; // 가로 중앙 정렬
    document.body.style.alignItems = 'center';     // 세로 중앙 정렬
    document.body.style.minHeight = '100vh';        // body가 뷰포트 전체 높이를 차지하도록
    document.body.style.overflow = 'hidden';        // 캔버스 크기가 뷰포트를 넘어갈 경우 스크롤바 방지

    const gameCanvas = document.createElement('canvas');
    gameCanvas.id = 'gameCanvas';
    gameCanvas.style.display = 'block'; // 캔버스 아래 추가 공백 제거
    gameCanvas.style.border = '2px solid #555'; // 캔버스 테두리 추가 (선택 사항)
    gameCanvas.style.boxShadow = '0 0 15px rgba(0,0,0,0.5)'; // 캔버스 그림자 추가 (선택 사항)
