...
interface GameData {
    ...
    ui: {
        ...
        titleScreenTextYOffsetFromBottom: number; // NEW: Offset from bottom for "Press any key to start" text
        titleScreenBlinkFrequency: number;     // NEW: Frequency for blinking "Press any key to start" text
...
interface Item extends GameObject {
    type: string; // e.g., "magnet", "special_attack", "health_potion"
    assetName: string;
...
function dropItem(x: number, y: number): void {
    ...
    if (Math.random() < gameData.gameplay.itemDropChanceOnEnemyDefeat && items.length < gameData.gameplay.maxSimultaneousItems) {
        ...
        if (!itemTypeConfig) return; // No items defined or weights sum to 0
...
function drawLevelUpScreen(): void {
    ...
    // MODIFIED: Use levelUpText as a format string and replace '{level}'
    ...
    if (gameData.ui.levelUpOptions.length > 0) {
        ...
        // MODIFIED: Translated hardcoded part to Korean
        ctx.fillText(`[스페이스바] ${option.name}: ${option.description}`, canvas.width / 2, canvas.height / 2);
    } else {
        // MODIFIED: Translated hardcoded part to Korean
