...
let ctx;
let gameData;
let assets = { images: new Map(), sounds: new Map() };
let player;
let enemies = [];
let projectiles = [];
let experienceGems = [];
let items = []; // NEW: Array to hold spawned items
...
let finalSurvivalTime = 0; // MODIFIED: Stores the game time when GAME_OVER state is reached.
let cameraX = 0; // NEW: X-coordinate of the camera's top-left corner in world space
let cameraY = 0; // NEW: Y-coordinate of the camera's top-left corner in world space
...
// NEW: Update items lifetime and add attraction
function updateItems(dt) {
    for (let i = items.length - 1; i >= 0; i--) {
        ...
        if (item.currentLifetime <= 0) {
            ...
            continue; // Move to the next item, skip attraction for expired items
        }
        // NEW: Attract items towards player similar to experience gems
        const dx = player.x - item.x;
        const dy = player.y - item.y;
        const dist = Math.sqrt(dx * dx + dy * dy);
        // Attract items if player is within the attract radius (using currentMagnetAttractRadius)
        // Items are collected by the collision check, so just move them here.
        if (dist < player.currentMagnetAttractRadius && dist > 0) {
            const attractSpeed = player.speed * 2; // Items move faster than player, same as gems
...
// NEW: Function to drop an item at a specific location when an enemy dies
function dropItem(x, y) {
    // Only drop an item if there's a configured chance and we haven't hit the max simultaneous items
    if (Math.random() < gameData.gameplay.itemDropChanceOnEnemyDefeat && items.length < gameData.gameplay.maxSimultaneousItems) {
        ...
        if (!itemTypeConfig)
            return; // No items defined or weights sum to 0
        items.push({
            x: x,
            y: y,
...
function checkCollisions() {
        ...
        for (let j = enemies.length - 1; j >= 0; j--) {
            ...
            if (isColliding(proj, enemy)) {
                ...
                if (enemy.health <= 0) {
                    ...
                    // NEW: Chance to drop an item when an enemy is defeated
    ...
    // NEW: Player-Item collisions (now that items are attracted, this handles final collection)
    for (let i = items.length - 1; i >= 0; i--) {
        const item = items[i];
        if (isColliding(player, item)) {
            applyItemEffect(item);
            items.splice(i, 1); // Remove collected item
            // Sound is now played inside applyItemEffect, to allow for specific sounds per item type
            // playSound('item_collect'); // REMOVED: Moved to applyItemEffect
        }
    }
}
...
function applyItemEffect(item) {
    switch (item.type) {
        ...
        case 'special_attack':
            ...
            break;
        case 'health_potion': // NEW: Health Potion item
            ...
            playSound('item_collect'); // Reuse general item collect sound
            break;
        default:
...
function playerLevelUp() {
    ...
    currentEffectiveEnemySpawnInterval = Math.max(gameData.gameplay.minEnemySpawnInterval, gameData.gameplay.baseEnemySpawnInterval * Math.pow(gameData.gameplay.spawnIntervalReductionFactorPerLevel, player.level - 1));
...
function drawTitleScreen() {
    ...
    // MODIFIED: "Press any key to start" text moved to bottom and blinks
...
function drawGameplay() {
    ...
    for (const gem of experienceGems) {
        ...
        if (gem.lifetime <= gameData.experienceGems.blinkThresholdSeconds) {
            ...
            // We use Math.floor(gameTimer * gameData.experienceGems.blinkFrequency) % 2 to alternate drawing/not drawing
...
function drawUI() {
    ...
    // No specific UI for health potion as it's an instant effect
}
