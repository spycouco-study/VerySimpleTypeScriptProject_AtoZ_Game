...
function startGameplay() {
    ...
    if (bgmBuffer) {
        ...
        playBGM(bgmBuffer, soundInfo?.volume || 1.0);
    } else {
...
function playBGM(buffer: AudioBuffer, volume: number) {
    if (currentBGM) {
        currentBGM.stop();
        currentBGM = null;
    }
...
function updateGameplay(deltaTime: number) {
    ...
    } else if (noteSpawnQueue.length === 0 && activeNotes.length === 0 && gameElapsedTime / 1000 > currentSong.notes[currentSong.notes.length - 1].time + 3) {
        // All notes spawned and processed, and a buffer time has passed after the last note
        transitionToState(GameState.GAME_OVER);
    }
}
