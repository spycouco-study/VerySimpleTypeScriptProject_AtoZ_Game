...
// Start a new game
function startGame() {
    console.log("Starting new game. Lives reset to", gameData.gameSettings.initialLives); // Added log for clarity
...
// Game over logic
function gameOver() {
    console.log("Game Over triggered. Final Score:", score, "Lives remaining:", lives); // Added log for clarity
...
function update(deltaTime: number) {
    if (currentGameState === GameState.PLAYING) {
        ...
        // Added fail-safe check: If lives somehow drop to 0 while in PLAYING state,
        // force a transition to GAME_OVER to ensure the game properly ends.
        if (lives <= 0) {
            console.log("Lives reached 0. Transitioning to Game Over state from update loop.");
            gameOver();
        }
    }
