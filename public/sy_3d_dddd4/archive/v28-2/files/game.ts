...
interface GameConfig {
    gameSettings: {
        ...
        horizontalResponsivenessFactor: number; // NEW: Controls player horizontal acceleration/deceleration
...
class Game {
    ...
    private updatePlayerMovement() {
        ...
        let moving = false;
        // Apply movement direction based on keys (W, S, A, D)
        if (this.keys['s']) { 
        ...
        // If pointer is not locked, or no movement keys are pressed, apply damping.
        // Otherwise, apply force towards the target speed.
        if (this.isPointerLocked && moving) {
            moveDirection.normalize(); // Normalize for consistent speed in all directions

            // Calculate the desired target horizontal velocity vector
            ...
            // Get current horizontal velocity vector from the physics body
            ...
            // Calculate the 'velocity error' - the difference between desired and current velocity
            const velocityError = new CANNON.Vec3();
            desiredTargetVelocity.vsub(currentHorizontalVelocity, velocityError);

            // Apply a force proportional to the velocity error.
            // This force will accelerate the player towards the desired target velocity.
            // The horizontalResponsivenessFactor acts as a gain, determining how quickly
            // the player's actual velocity catches up to the desired velocity.
            ...
            // If no movement keys are pressed OR pointer is not locked,
            // apply a damping force to bring horizontal velocity to zero.
            const currentHorizontalVelocity = new CANNON.Vec3(
                this.playerBody.velocity.x,
                0,
                this.playerBody.velocity.z
            ...
            // Apply force opposing the current velocity, proportional to the responsiveness factor.
            // This makes the player smoothly decelerate to a stop.
        ...
        // Always restore the original Y velocity to preserve jumping and gravity effects
