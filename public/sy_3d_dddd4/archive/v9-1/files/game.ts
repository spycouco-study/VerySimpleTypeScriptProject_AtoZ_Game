...
class Game {
    ...
    private updatePlayerMovement() {
        ...
        let moving = false;
        if (this.keys['w']) { // Original: Forward. Changed to: Backward
            desiredHorizontalVelocity.x -= cameraDirection.x;
            desiredHorizontalVelocity.z -= cameraDirection.z;
            moving = true;
        }
        if (this.keys['s']) { // Original: Backward. Changed to: Forward
            desiredHorizontalVelocity.x += cameraDirection.x;
            desiredHorizontalVelocity.z += cameraDirection.z;
            moving = true;
        }
        if (this.keys['a']) { // Original: Strafe left. Changed to: Strafe Right
            desiredHorizontalVelocity.x += cameraRight.x; 
            desiredHorizontalVelocity.z += cameraRight.z;
            moving = true;
        }
        if (this.keys['d']) { // Original: Strafe right. Changed to: Strafe Left
            desiredHorizontalVelocity.x -= cameraRight.x;
            desiredHorizontalVelocity.z -= cameraRight.z;
            moving = true;
        }
