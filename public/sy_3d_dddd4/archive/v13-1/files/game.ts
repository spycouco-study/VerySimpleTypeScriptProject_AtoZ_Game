...
class Game {
    ...
    private updatePlayerMovement() {
        ...
        if (this.keys['s']) { // Move Backward
            moveDirection.sub(cameraDirection);
            moving = true;
        }
        if (this.keys['a']) { // Strafe Left
            moveDirection.sub(cameraRight); // Subtracting 'right' vector moves left
            moving = true;
        }
        if (this.keys['d']) { // Strafe Right
            moveDirection.add(cameraRight); // Adding 'right' vector moves right
            moving = true;
        }
