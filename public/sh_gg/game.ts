...
class Game {
    canvas: HTMLCanvasElement; // No longer nullable; constructor will throw if not found
    ctx: CanvasRenderingContext2D; // No longer nullable
    ...
    constructor(canvasId: string, config: GameConfig) {
        this.config = config; // Set config first so error screens can use it if needed.

        // Robust canvas and context initialization
        const canvasElement = document.getElementById(canvasId);
        if (!(canvasElement instanceof HTMLCanvasElement)) {
            const errorMessage = `FATAL ERROR: Canvas element with ID "${canvasId}" not found or is not an HTMLCanvasElement. Please ensure your HTML contains <canvas id="${canvasId}"></canvas>.`;
            console.error(errorMessage);
            // This error will be caught by the global fetch().catch, which will draw to a temp canvas
        ...
        if (!context) {
            const errorMessage = `FATAL ERROR: Could not get 2D rendering context for canvas ID "${canvasId}". Your browser might not support it.`;
            console.error(errorMessage);
            throw new Error(errorMessage);
        }
        this.ctx = context;

        // If we reach here, canvas and context are valid
...
document.addEventListener('DOMContentLoaded', () => {
    fetch('data.json')
        ...
        .catch(error => {
            console.error('Error loading game data or initializing game:', error);
            // If fetch fails or new Game() throws an error, try to display a fatal error message.
            const canvas = document.getElementById('gameCanvas') as HTMLCanvasElement;
            if (canvas) {
                const ctx = canvas.getContext('2d');
                if (ctx) {
                    // Set default size if config not loaded
                    ...
                    ctx.fillText('FATAL ERROR: Game initialization failed!', canvas.width / 2, canvas.height / 2 - 20);
                    ctx.font = '20px Arial';
                    ctx.fillText('Check console for details.', canvas.width / 2, canvas.height / 2 + 20);
                    // Also display the specific error message if it's an Error object
                    if (error instanceof Error) {
                        ctx.fillText(`Detail: ${error.message}`, canvas.width / 2, canvas.height / 2 + 60);
                    }
                }
            } else {
                // If even the canvas is missing, we can only log to console.
                ...
                errorDiv.innerText = `FATAL ERROR: Cannot initialize game. ${error instanceof Error ? error.message : 'Unknown error.'} See console for details.`;
