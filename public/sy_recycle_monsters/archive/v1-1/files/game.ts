...
interface GameSettings {
    ...
    mapHeight: number;
    playerSpeed: number;
    recyclableSpawnCount: number;
    wildMonsterSpawnCount: number;
    titleScreenText: string;
    gameClearText: string;
    gameOverText: string;
    monsterDespawnDelayMs: number;
}
...
interface PlayerConfig {
    ...
    height: number;
    sprite: string;
    maxRecyclables: number;
    maxCompanionMonsters: number;
}

interface RecyclableConfig {
    id: string;
    name: string;
    sprite: string;
    width: number;
...
interface MonsterCombination {
    inputRecyclables: string[];
    outputMonsterId: string;
    count: number;
}
...
interface UIMessages {
    ...
    monsterCombinePrompt: string;
    notEnoughRecyclables: string;
    monsterCreated: string;
    noMonstersToControl: string;
    companionLimitReached: string;
    wildMonsterCleared: string;
    playerMonsterDefeated: string;
}
...
interface GameData {
    ...
    player: PlayerConfig;
    recyclables: RecyclableConfig[]; // Modified: Changed type from Recyclable[] to RecyclableConfig[]
    playerMonsters: MonsterConfig[];
    monsterCombinations: MonsterCombination[];
    wildMonsters: MonsterConfig[];
    mapCollisions: CollisionRect[];
 ...
 */
function playSound(name: string, loop?: boolean): void {
...
class GameObject {
    ...
    constructor(x: number, y: number, width: number, height: number, spriteName: string) {
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
...
class Player extends GameObject {
    speed: number;
    inventory: Map<string, number>; // Map<recyclableId, count>
    maxRecyclables: number;
    companionMonsters: PlayerMonster[];
    maxCompanionMonsters: number;
     ...
     */
    update(deltaTime: number, keysPressed: { [key: string]: boolean }, mapCollisions: CollisionRect[], mapWidth: number, mapHeight: number): void {
        let newX = this.x;
        let newY = this.y;

        if (keysPressed['ArrowUp']) newY -= this.speed * deltaTime;
        if (keysPressed['ArrowDown']) newY += this.speed * deltaTime;
        if (keysPressed['ArrowLeft']) newX -= this.speed * deltaTime;
        if (keysPressed['ArrowRight']) newX += this.speed * deltaTime;
     ...
     */
    addRecyclable(recyclableId: string): void {
        let currentTotal = 0;
     ...
     */
    useRecyclables(recyclableId: string, count: number): boolean {
        const currentCount = this.inventory.get(recyclableId) || 0;
...
class Recyclable extends GameObject {
    ...
    constructor(x: number, y: number, config: RecyclableConfig) {
...
class Monster extends GameObject {
    ...
    maxCombatCooldown: number;
    level: number;
    despawnTimer: number;

    constructor(x: number, y: number, config: MonsterConfig) {
        super(x, y, config.width, config.height, config.sprite);
        this.id = config.id;
        this.name = config.name;
    ...
    startDespawn(delayMs: number): void {
...
class PlayerMonster extends Monster {
    levelUpHp: number;
    levelUpAtk: number;
    followOffset: { x: number; y: number; }; // Relative position to player
    leader: Player;
    
    constructor(x: number, y: number, config: MonsterConfig, leader: Player, followOffset: {x:number, y:number}) {
        super(x, y, config);
    ...
    update(deltaTime: number, wildMonsters: WildMonster[], mapCollisions: CollisionRect[], mapWidth: number, mapHeight: number): void {
        if (!this.isAlive) return;
        ...
        let nearestWildMonster: WildMonster | null = null;
        let minDistanceSq = Infinity;
        for (const wildMon of wildMonsters) {
            if (!wildMon.isAlive) continue;
...
class WildMonster extends Monster {
    ...
    update(deltaTime: number, mapWidth: number, mapHeight: number, mapCollisions: CollisionRect[], playerMonsters: PlayerMonster[]): void {
        if (!this.isAlive) return;
        ...
        let nearestPlayerMonster: PlayerMonster | null = null;
        let minDistanceSq = Infinity;
        for (const playerMon of playerMonsters) {
            if (!playerMon.isAlive) continue;
...
class Game {
    ...
    isCombiningMonsters: boolean;
    selectedRecyclableForCombine: string | null;
    combineCount: number;
    currentUIMessage: string;
     ...
     */
    spawnRecyclables(count: number): void {
     ...
     */
    spawnWildMonsters(count: number): void {
        for (let i = 0; i < count; i++) {
     ...
     */
    tryCombineMonster(selectedOptionIndex: number): void {
        ...
        if (recipe) {
            ...
            let canCombine = true;
            const requiredItems: Map<string, number> = new Map();
            ...
            if (canCombine) {
                ...
                if (monsterConfig) {
                    ...
                    const newMonster = new PlayerMonster(
     ...
     */
    drawCombineUI(): void {
        if (!this.isCombiningMonsters) return;
