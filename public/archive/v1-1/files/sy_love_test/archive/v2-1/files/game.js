...
async function initGame() {
    ...
    try {
        ...
        gameData.playerStats.forEach(stat => {
            const statKey = `starting${capitalize(stat)}`;
...
function update(deltaTime) {
    if (gameState.isLoading || gameState.isGameOver || gameState.waitingForStart)
        return;
}
function render() {
    if (!ctx)
...
function drawButton(text, x, y, width, height, index) {
...
function processChoice(choiceIndex) {
    if (choiceIndex < 0 || choiceIndex >= gameState.currentChoices.length)
        return;
...
function applyEffects(effects) {
    ...
    if (positiveEffect)
        assetLoader.playAudio("sfx_positive");
    if (negativeEffect)
        assetLoader.playAudio("sfx_negative");
}
...
function showEnding() {
    ...
    gameData.npcs.forEach(npc => {
        ...
        if (affection >= 100)
            relationshipStatus = "매우 좋은 관계!";
        else if (affection >= 50)
            relationshipStatus = "친밀한 관계";
        else if (affection < 0)
            relationshipStatus = "거리가 있는 관계...";
