        ...
        cellsToExplode.forEach(cellKey => {
            ...
            if (cx === gridX && cy === gridY) {
                ...
                if (dirType) {
                    ...
                    const nextX = cx + Math.sign(dx);
                    const nextY = cy + Math.sign(dy);

                    if (nextX >= 0 && nextX < game.grid[0].length && nextY >= 0 && nextY < game.grid.length) {
                        const nextCellKey = `${nextX},${nextY}`;
                        if (cellsToExplode.has(nextCellKey)) {
            ...
            if (game.grid[cy][cx] === TileType.BREAKABLE) {
                ...
                if (Math.random() < game.config.itemSpawnChance) {
                    game.items.push(new Item(cx * game.tileSize, cy * game.tileSize, game.tileSize, game.config.explosionDurationMs * 2)); // Items last longer
                }
            }
...
class Game {
    ...
    constructor(canvasId: string, config: GameConfig) {
        ...
        this.loadAssets().then(() => {
            console.log('Assets loaded. Starting title screen.');
