...
interface GameConfig {
    ...
    groundHeight: number;
    gravity: number;
    obstacleSpawnIntervalMin: number;
    obstacleSpawnIntervalMax: number;
    scorePerSecond: number;
    playerScreenXRatio: number;
    stompScore: number;
...
interface PlayerConfig {
    ...
    moveSpeed: number;
    maxJumps: number; // Added for double jump functionality
...
class Player {
    ...
    height: number;
    velocityY: number;
    isJumping: boolean; // Indicates if player is airborne
    jumpCount: number;  // How many jumps have been performed since last on ground
    maxJumps: number;   // Max jumps allowed (from config)
    currentFrame: number;
    ...
    constructor(config: PlayerConfig, startY: number) {
        this.config = config;
...
class Obstacle {
    ...
    constructor(config: ObstacleConfig, worldX: number, groundY: number) {
        this.config = config;
...
class Background {
    ...
    constructor(canvasWidth: number, canvasHeight: number, imageName: string) {
...
class Game {
    ...
    lastTime: number;
    gameState: 'START_SCREEN' | 'RUNNING' | 'GAME_OVER'; // New state property
    score: number;
    ...
    constructor(canvasId: string, dataUrl: string = 'data.json') {
    ...
    async loadDataAndAssets(dataUrl: string): Promise<void> {
    ...
    render() {
        ...
        this.backgrounds.forEach(bg => {
            ...
            if (bgAsset) {
                ...
                while (currentX < this.canvas.width) {
                    this.drawSprite(bg.imageName, 0, 0, bgAsset.width, bgAsset.height,
                                    currentX, 0, bg.width, bg.height);
                    currentX += bg.width;
                }
            }
        });
        ...
        if (groundImageInfo) {
            ...
            while (currentX < this.canvas.width) {
                this.drawSprite('ground', 0, 0, groundImageInfo.width, groundImageInfo.height,
                                currentX, this.groundY, groundImageInfo.width, this.gameData.game.groundHeight);
        ...
        this.obstacles.forEach(obstacle => {
            ...
            if (obstacleAsset) {
                this.drawSprite('obstacles',
                                obstacleAsset.sprite.x, obstacleAsset.sprite.y,
                                obstacleAsset.sprite.width, obstacleAsset.sprite.height,
                                obstacle.x - this.worldOffsetX, obstacle.y,
        ...
        if (playerAsset) {
            ...
            this.drawSprite('player',
                            ...
                            this.player.x - this.worldOffsetX, this.player.y,
    ...
    drawSprite(imageName: string, sx: number, sy: number, sWidth: number, sHeight: number, dx: number, dy: number, dWidth: number, dHeight: number) {
    ...
    getRandomObstacleSpawnTime(): number {
