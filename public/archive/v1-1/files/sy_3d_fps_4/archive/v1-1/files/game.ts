...
function setupPlayer(): void {
    ...
    controls = new PointerLockControls(camera, canvas); // Controls attach to camera
    scene.add(controls.object); // controls.object is an Object3D containing the camera.
}
...
function startGame(): void {
    ...
    // Reset player
    player.reset();
    controls.object.position.copy(player.body.position as unknown as THREE.Vector3);
    controls.object.position.y += config.player.cameraOffset.y;
    controls.object.rotation.y = 0; // Reset horizontal rotation
    controls.object.children[0].rotation.x = 0; // Reset vertical rotation (pitch)
...
function animate(currentTime: DOMHighResTimeStamp): void {
    ...
    if (gameState === GameState.PLAYING) {
        ...
        // Synchronize camera/controls position with player physics body
        controls.object.position.copy(player.body.position as unknown as THREE.Vector3);
        controls.object.position.y += config.player.cameraOffset.y; // Adjust for camera height
...
function shootBullet(): void {
    ...
    // Bullet spawn position (slightly in front of camera)
    ...
    startPosition.y += config.player.cameraOffset.y; // Adjust for camera height
