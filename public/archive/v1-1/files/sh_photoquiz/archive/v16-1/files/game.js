...
let gameData; // Will hold the parsed data from data.json
let assets = { images: new Map(), sounds: new Map() };
let currentCelebrities = []; // Shuffled list of celebrities for rounds
let currentCelebrity = null; // The celebrity for the current round
let currentCelebrityImage = null; // The loaded image for the current celebrity
let timeRemaining = 0; // Time in milliseconds for the current round
let lives = 0;
let score = 0;
let message = null; // Message like "Correct!", "Wrong!", "Time's Up!"
let messageColor = '';
let messageTimer = 0; // How long to display the message (in seconds)
let lastFrameTime = 0; // For delta time calculation in the game loop
...
function positionInputBox() {
    if (!inputElement || !gameData || !canvas)
        return;
...
function checkGuess() {
    if (!currentCelebrity || !inputElement)
        return;
...
function render() {
    if (!ctx || !gameData)
        return; // Ensure context and gameData are available
    ...
    if (currentGameState === GameState.TITLE) {
        ...
        // Split the text by the newline character to render multiple lines
        const lines = gameData.gameSettings.titleScreenText.split('\n');
        const fontSizeNum = parseInt(gameData.gameSettings.fontSizeTitle, 10); // Extract font size number (e.g., 48 from "48px")
        const lineHeight = fontSizeNum; // Use font size directly for line spacing
        ...
        ctx.fillText(`Time: ${Math.max(0, Math.ceil(timeRemaining / 1000))}`, canvas.width / 2, gameData.gameSettings.uiTopY);
        // Draw lives
