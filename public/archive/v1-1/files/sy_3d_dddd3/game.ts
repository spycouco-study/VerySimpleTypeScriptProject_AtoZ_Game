...
enum GameState {
    TITLE,
    PLAYING,
    PAUSED // Added PAUSED state for when pointer lock is released
}
...
function initializeGame() {
    ...
    // Listen for pointer lock change events to pause/unpause game state
...
function onKeyDown(event: KeyboardEvent) {
    // WASD movement keys only active when PLAYING
    ...
    // ESC key handles releasing pointer lock to pause the game
    if (event.code === 'Escape') {
        if (document.pointerLockElement === canvas) {
            // If pointer is currently locked (game is PLAYING), release it
...
function onKeyUp(event: KeyboardEvent) {
    if (gameState !== GameState.PLAYING) return; // Only process key up for movement when playing
    switch (event.code) {
...
function onClick() {
    if (gameState === GameState.TITLE) {
        ...
        // If the game is paused (and thus pointer is unlocked), a click should re-lock it
        canvas.requestPointerLock();
    }
    // No action if gameState is PLAYING and pointer is already locked (e.g., clicking on screen during gameplay)
}

function onPointerLockChange() {
    if (document.pointerLockElement === canvas) {
        // Pointer is locked. If the game was paused, it means we are resuming.
        if (gameState === GameState.PAUSED) {
            gameState = GameState.PLAYING; // Resume game
            console.log("Game Resumed (Pointer Locked)");
        }
    } else {
        // Pointer is unlocked. If the game was playing, it means it's now paused.
        if (gameState === GameState.PLAYING) {
            gameState = GameState.PAUSED; // Pause game
            console.log("Game Paused (Pointer Unlocked)");
            // Optionally, you could show a pause menu UI here.
        }
    }
}
...
function animate() {
    ...
    // Only update game logic (physics, player movement, camera position) when in PLAYING state
    if (gameState === GameState.PLAYING) {
    ...
    // Render the scene with the camera (always render, even if paused, to show the static scene)
