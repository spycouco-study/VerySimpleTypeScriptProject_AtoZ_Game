...
class TetrisGame {
    constructor(canvasId, data) {
        ...
        this.keyRepeatInterval = 100; // ms
        this.initialKeyDelay = 200; // ms
        ...
        if (gridTile) {
            for (let y = 0; y < this.settings.gridHeight; y++) {
                for (let x = 0; x < this.settings.gridWidth; x++) {
                    this.ctx.drawImage(gridTile, gridOffsetX + x * this.settings.blockSize, gridOffsetY + y * this.settings.blockSize, this.settings.blockSize, this.settings.blockSize);
                }
            }
        }
    ...
    drawBlock(gridX, gridY, textureName, offsetX, offsetY) {
        const image = this.assetLoader.getImage(textureName);
        if (image) {
            this.ctx.drawImage(image, offsetX + gridX * this.settings.blockSize, offsetY + gridY * this.settings.blockSize, this.settings.blockSize, this.settings.blockSize);
        }
        else {
            // Fallback if texture not found
            this.ctx.fillStyle = 'gray'; // Use a fallback color
            this.ctx.fillRect(offsetX + gridX * this.settings.blockSize, offsetY + gridY * this.settings.blockSize, this.settings.blockSize, this.settings.blockSize);
            this.ctx.strokeStyle = 'black';
            this.ctx.strokeRect(offsetX + gridX * this.settings.blockSize, offsetY + gridY * this.settings.blockSize, this.settings.blockSize, this.settings.blockSize);
        }
    }
    drawCurrentPiece() {
                ...
                if (this.currentPiece.shape[r][c]) {
                    ...
                    if (blockY >= 0) { // Don't draw ghost above the visible grid
                        ...
                        if (image) {
                            ...
                            this.ctx.drawImage(image, gridOffsetX + blockX * this.settings.blockSize, gridOffsetY + blockY * this.settings.blockSize, this.settings.blockSize, this.settings.blockSize);
                            this.ctx.restore();
                        }
                    }
                }
        ...
        for (let r = 0; r < this.currentPiece.shape.length; r++) {
            for (let c = 0; c < this.currentPiece.shape[r].length; c++) {
                if (this.currentPiece.shape[r][c]) {
                    this.drawBlock(this.currentPiece.x + c, this.currentPiece.y + r, this.currentPiece.texture, gridOffsetX, gridOffsetY);
                }
            }
        }
    }
            ...
            for (let r = 0; r < this.nextPiece.shape.length; r++) {
                for (let c = 0; c < this.nextPiece.shape[r].length; c++) {
                    if (this.nextPiece.shape[r][c]) {
                        ...
                        if (image) {
                            this.ctx.drawImage(image, nextPieceOffsetX + c * this.settings.blockSize, nextPieceOffsetY + r * this.settings.blockSize, this.settings.blockSize, this.settings.blockSize);
                        }
                    }
                }
            }
    ...
    clearLines() {
        ...
        if (linesClearedThisTurn > 0) {
            ...
            switch (linesClearedThisTurn) {
                case 1:
                    scoreMultiplier = this.settings.scorePerLine;
                    break;
                case 2:
                    scoreMultiplier = this.settings.scorePerDoubleLine;
                    break;
                case 3:
                    scoreMultiplier = this.settings.scorePerTripleLine;
                    break;
                case 4:
                    scoreMultiplier = this.settings.scorePerTetris;
                    break;
            }
