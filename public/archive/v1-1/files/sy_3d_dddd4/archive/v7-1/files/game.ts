...
class Game {
    ...
    private onMouseMove(event: MouseEvent) {
        ...
        if (this.state === GameState.PLAYING && this.isPointerLocked) {
            ...
            // Corrected: Changed from += to -= to make mouse Y-axis movement intuitive.
            // Mouse UP (movementY < 0) now decreases movementY, thus increases cameraPitch -> looks up.
    ...
    private onPointerLockChange() {
        if (document.pointerLockElement === this.canvas ||
            ...
            (document as any).webkitPointerLockElement === this.canvas) { // Corrected webkitPointerLockElement
    ...
    private updatePlayerMovement() {
        ...
        // The original calculation `cameraRight.crossVectors(this.camera.up, cameraDirection)`
        // resulted in a vector pointing to the LEFT of the camera's view.
        // To fix A/D keys, we either reverse the cross product or reverse the signs below.
        // Reversing the cross product: `cameraRight.crossVectors(cameraDirection, this.camera.up);`
        // Or reversing the signs below, which is simpler given the current `cameraRight` definition:
        cameraRight.crossVectors(this.camera.up, cameraDirection); // This gives a vector pointing LEFT
        ...
        if (this.keys['a']) { // Strafe left - Changed from -= to += to correct direction
            desiredHorizontalVelocity.x += cameraRight.x; 
            desiredHorizontalVelocity.z += cameraRight.z;
            moving = true;
        }
        if (this.keys['d']) { // Strafe right - Changed from += to -= to correct direction
            desiredHorizontalVelocity.x -= cameraRight.x;
            desiredHorizontalVelocity.z -= cameraRight.z;
            moving = true;
        }
