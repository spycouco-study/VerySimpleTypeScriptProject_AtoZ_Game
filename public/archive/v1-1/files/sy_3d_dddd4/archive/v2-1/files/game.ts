...
class Game {
    ...
    private async init() {
        ...
        // Removed: this.canvas.addEventListener('click', this.handleCanvasClick.bind(this)); // This was the problematic line
    ...
    private setupTitleScreen() {
        ...
        // Add event listener directly to the overlay to capture clicks
    ...
    private createPlayer() {
        ...
        // Set initial camera position relative to player's physics body.
        // It will be continuously updated in the animate loop.
    ...
    private onMouseMove(event: MouseEvent) {
        ...
        if (this.state === GameState.PLAYING && this.isPointerLocked) {
            ...
            // Apply horizontal rotation (yaw) to the camera around its local Y-axis
            this.camera.rotation.y -= movementX * this.config.gameSettings.mouseSensitivity;

            // Apply vertical rotation (pitch) and clamp it to prevent the camera from flipping over
    ...
    private onPointerLockChange() {
        if (document.pointerLockElement === this.canvas ||
            ...
            // In a more complex game, you might pause the game or show a menu here.
            // For this simple game, we'll just allow gameplay to continue without mouse look.
        }
    }
    ...
    private updatePlayerMovement() {
        ...
        // Start with current vertical velocity, and zero horizontal for fresh calculation
        const targetVelocity = new CANNON.Vec3(0, currentVelocity.y, 0);

        // Get camera's forward and right vectors to determine movement direction relative to view
        const cameraDirection = new THREE.Vector3();
        this.camera.getWorldDirection(cameraDirection); // Get forward vector in world coordinates
        cameraDirection.y = 0; // Flatten the vector to restrict movement to the horizontal plane
        cameraDirection.normalize();

        const cameraRight = new THREE.Vector3();
        cameraRight.crossVectors(this.camera.up, cameraDirection); // Calculate the right vector
        cameraRight.normalize();

        // Apply desired velocity based on key presses
        ...
        if (this.keys['a']) { // Strafe left
            targetVelocity.x -= cameraRight.x * playerSpeed;
            targetVelocity.z -= cameraRight.z * playerSpeed;
        }
        if (this.keys['d']) { // Strafe right
            targetVelocity.x += cameraRight.x * playerSpeed;
            targetVelocity.z += cameraRight.z * playerSpeed;
        }

        // Smoothly interpolate current velocity towards target velocity for smoother movement.
        // This helps prevent jerky stop/start movements common with direct velocity setting.
        const lerpFactor = 0.2; // Determines how quickly the velocity changes
        ...
        // Update camera position to follow the player's physics body, adding an eye-level offset
        this.camera.position.copy(this.playerBody.position as unknown as THREE.Vector3);
