...
class Game {
    ...
    private updatePlayerMovement() {
        ...
        let moving = false;
        // W and S key functionality are swapped to standard WASD.
        // 'w' key now moves forward.
        if (this.keys['w']) { 
            moveDirection.add(cameraDirection);
            moving = true;
        }
        // 's' key now moves backward.
        if (this.keys['s']) { 
            moveDirection.sub(cameraDirection);
            moving = true;
        }
        // A and D key functionality are swapped to standard WASD as requested.
        // 'a' key now strafes left.
        if (this.keys['a']) { 
            moveDirection.sub(cameraRight); 
            moving = true;
        }
        // 'd' key now strafes right.
        if (this.keys['d']) { 
