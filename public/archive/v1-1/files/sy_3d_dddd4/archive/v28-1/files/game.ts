...
class Game {
    ...
    private updatePlayerMovement() {
        ...
        // FIX: Explicitly clamp the horizontal velocity to prevent it from exceeding playerSpeed,
        // which can sometimes happen due to subtle physics engine interactions, especially during jumps
        // or transitions between ground and air. This ensures consistent maximum horizontal speed.
        const currentHorizontalSpeedSq = this.playerBody.velocity.x * this.playerBody.velocity.x + 
        ...
        if (currentHorizontalSpeedSq > playerSpeedSq) {
            ...
            if (currentHorizontalSpeed > 0) { // Prevent division by zero if velocity is (0,0,0)
