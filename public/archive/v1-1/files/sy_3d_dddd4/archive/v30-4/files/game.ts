...
class Game {
    ...
    // ADDED: Cannon.js materials for friction control
    private playerMaterial!: CANNON.Material;
    private groundMaterial!: CANNON.Material;
    private defaultObjectMaterial!: CANNON.Material; // For placed objects
    ...
    private async init() {
        ...
        // ADDED: Define physics materials and contact materials for friction control
        this.playerMaterial = new CANNON.Material('playerMaterial');
        this.groundMaterial = new CANNON.Material('groundMaterial');
        this.defaultObjectMaterial = new CANNON.Material('defaultObjectMaterial'); // For general static/dynamic objects

        // Create a ContactMaterial for player-ground interaction with no friction
        const playerGroundContactMaterial = new CANNON.ContactMaterial(
            this.playerMaterial,
            this.groundMaterial,
            {
                friction: 0,    // No friction between player and ground
                restitution: 0.1 // A little bounce
            }
        );
        this.world.addContactMaterial(playerGroundContactMaterial);

        // Create a ContactMaterial for player-placed object interaction with no friction
        const playerObjectContactMaterial = new CANNON.ContactMaterial(
            this.playerMaterial,
            this.defaultObjectMaterial,
            {
                friction: 0,    // No friction between player and other objects
                restitution: 0.1 // A little bounce
            }
        );
    ...
    private createPlayer() {
        ...
        this.playerBody = new CANNON.Body({
            ...
            material: this.playerMaterial // Assign the player's material
        });
    ...
    private createGround() {
        ...
        this.groundBody = new CANNON.Body({
            ...
            shape: groundShape,
            material: this.groundMaterial // Assign the ground's material
        });
    ...
    private createPlacedObjects() {
        ...
        this.config.gameSettings.placedObjects.forEach(objConfig => {
            ...
            const body = new CANNON.Body({
                ...
                shape: shape,
                material: this.defaultObjectMaterial // Assign default object material
            });
            if (objConfig.rotationY !== undefined) {
    ...
    private updatePlayerMovement() {
        ...
        if (moving) {
            ...
            // If no movement keys are pressed, and friction is set to 0 via ContactMaterial,
            // the player's horizontal velocity will naturally decay only due to other physics forces (like air drag if modeled)
            // or remain constant if not interacting with anything.
            // The previous 'frictionFactor' manual deceleration is removed to achieve a truly frictionless feel.
        }
