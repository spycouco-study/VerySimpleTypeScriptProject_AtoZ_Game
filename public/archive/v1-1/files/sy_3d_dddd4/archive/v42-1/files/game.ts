...
class Enemy {
    ...
    private isPendingDestroy: boolean = false; // Flag to prevent multiple destroy calls
    ...
    update(deltaTime: number, playerBody: CANNON.Body) {
        if (this.isPendingDestroy) return; // Don't update if scheduled for destruction
    ...
    takeDamage(amount: number) {
        if (this.isPendingDestroy) return; // Cannot take damage if already being destroyed
     ...
     */
    destroy() {
        if (this.isPendingDestroy) return; // Prevent multiple additions to destroy list
        this.isPendingDestroy = true;
        this.game.enemiesToDestroy.push(this); // Add to game's destruction queue
        this.game.removeEnemy(this); // Remove from active enemies list immediately
     ...
     */
    performDestroy() {
        if (!this.isPendingDestroy) return; // Should only be called for pending destruction
...
class Bullet {
    ...
    public readonly ownerBody: CannonBodyWithUserData; // To ignore initial collision with shooter
    private isPendingDestroy: boolean = false; // Flag to prevent multiple destroy calls
    ...
    destroy() {
        if (this.isPendingDestroy) return; // Prevent multiple additions to destroy list
        this.isPendingDestroy = true;
        this.game.bulletsToDestroy.push(this); // Add to game's destruction queue
        this.game.removeBullet(this); // Notify Game class to remove from its list (from active array)
    }
...
class Game {
    ...
    bulletsToDestroy: Bullet[] = []; // Bullets to be destroyed after physics step
    enemiesToDestroy: Enemy[] = []; // Enemies to be destroyed after physics step
            ...
            if (bulletInstance && targetBody) {
                ...
                switch (targetBody.userData?.type) {
                    case GameObjectType.ENEMY:
                        ...
                        bulletInstance.destroy(); // Schedule bullet for destruction after hitting enemy
                        break;
                    case GameObjectType.STATIC_OBJECT:
                        bulletInstance.destroy(); // Schedule bullet for destruction after hitting static environment
                        break;
    ...
    private animate(time: DOMHighResTimeStamp) {
        ...
        if (this.state === GameState.PLAYING) {
            ...
            this.processDestroyQueue();          // NEW: Process deferred destructions after physics
    ...
    private processDestroyQueue() {
        ...
        this.bulletsToDestroy.length = 0; // Clear the queue

        for (const enemy of this.enemiesToDestroy) {
            enemy.performDestroy();
        }
        this.enemiesToDestroy.length = 0; // Clear the queue
    }
