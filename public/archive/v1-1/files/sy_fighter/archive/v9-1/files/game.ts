...
class InputHandler {
    ...
    private justPressedKeys: Set<string> = new Set(); // NEW: To track keys pressed down only in the current frame
    private game: Game;
    ...
    private onKeyDown = (e: KeyboardEvent) => {
        if (!e.repeat) { // Ignore key repeat
            ...
            this.justPressedKeys.add(e.code); // NEW: Add to justPressedKeys

            // Handle game state specific input (these are effectively 'just pressed' due to !e.repeat)
    ...
    private onKeyUp = (e: KeyboardEvent) => {
        ...
        // No need to remove from justPressedKeys here, clearFrameState() will handle it.
    }
    ...
    // NEW: Check if a key was just pressed in the current frame
    isKeyJustPressed(code: string): boolean {
        return this.justPressedKeys.has(code);
    }

    // NEW: Consume a 'just pressed' key so it doesn't trigger again until a new press
    consumeKeyJustPressed(code: string): void {
        this.justPressedKeys.delete(code);
    }

    // NEW: Clear all 'just pressed' keys at the beginning of each frame
    clearFrameState() {
    ...
    clearKeys() {
        ...
        this.justPressedKeys.clear(); // NEW: Also clear justPressedKeys when resetting game state
    }
}
...
class Character {
    ...
    handleInput(input: InputHandler) {
        ...
        // Blocking takes precedence over movement and attacks (uses isKeyDown for continuous action)
        ...
        // Movement (uses isKeyDown for continuous action)
        ...
        // Jump (NEW: uses isKeyJustPressed for one-shot action)
        if (input.isKeyJustPressed(this.playerControls.jump) && this.characterState !== CharacterState.JUMPING) {
            ...
            input.consumeKeyJustPressed(this.playerControls.jump); // Consume the jump press
        }

        // Attacks (NEW: uses isKeyJustPressed for one-shot action)
        if (input.isKeyJustPressed(this.playerControls.lightPunch)) {
            this.attack('light_punch');
            input.consumeKeyJustPressed(this.playerControls.lightPunch); // Consume the punch press
        } else if (input.isKeyJustPressed(this.playerControls.heavyKick)) {
            this.attack('heavy_kick');
            input.consumeKeyJustPressed(this.playerControls.heavyKick); // Consume the kick press
        }
    }
    ...
    update(dt: number) {
        switch (this._state) {
            ...
            case GameState.PLAYING:
                // NEW: Clear justPressed keys at the start of the frame for character input
