...
// Interfaces for data.json structure and internal types
interface Rect {
    x: number;
    y: number;
    width: number;
...
interface GameSettings {
    ...
    [key: string]: any; // Allow indexing with strings for damage values
}
...
interface AnimationConfig {
    ...
    frames: number;
    frameDelay: number;
    hitbox?: Rect; // Optional hitbox for attacks
    damage?: string; // Key in GameSettings for damage amount (e.g., "lightPunchDamage")
    sfx?: string; // Sound effect name
}
...
class AssetLoader {
    ...
    private loadImage(imageAsset: ImageAsset): Promise<HTMLImageElement> {
    ...
    private loadSound(soundAsset: SoundAsset): Promise<HTMLAudioElement> {
        return new Promise((resolve, reject) => {
            ...
            audio.oncanplaythrough = () => { // Ensure sound is fully loaded
...
class InputHandler {
    ...
    private justPressedKeys: Set<string> = new Set(); // New set for keys pressed this frame only
    private game: Game;
    ...
    clearKeys() { // This method is used for full reset, like when game state changes.
...
class Character {
    ...
    public config: CharacterConfig; // Made public to access name for winner display
...
class Game {
    ...
    private winnerName: string | null = null; // Added to store the winner's name
    ...
    async init() {
        ...
        try {
            ...
            gameConfig = await response.json() as GameConfig; // Assign to global gameConfig
