...
let hoveredBackButton: boolean = false; // New: Tracks hover state for the back button
let lastTypeSoundTime: number = 0;
let sceneHistory: string[] = []; // New: Stack to keep track of visited scenes for back navigation
...
async function init() {
    ...
    canvas.addEventListener('click', () => {
        ...
        if (currentGameState === GameState.TITLE) {
            ...
            playSFX("click_sfx");
            resetTextTypingState(); // Use renamed function for initial scene
...
// Renamed from resetSceneState to reflect its purpose of resetting text typing
function resetTextTypingState() {
...
// New: Function for back button action
function handleBackButton() {
    if (sceneHistory.length > 0) {
        console.log("Executing handleBackButton. History:", sceneHistory); // DEBUG LOG
        playSFX("click_sfx");
        const prevSceneId = sceneHistory.pop()!;
        goToScene(prevSceneId, false); // Do not push current scene to history when going back
    } else {
        console.log("handleBackButton called, but history is empty. Cannot go back."); // DEBUG LOG
    }
}

function handlePresentationClick() {
    console.log("handlePresentationClick called. Mouse:", mouseX, mouseY); // DEBUG LOG
    ...
    if (isBackButtonClicked) {
        console.log("Back button area clicked!"); // DEBUG LOG
        handleBackButton();
...
function updatePresentation(deltaTime: number) {
    const currentScene = gameData.scenes[currentSceneIndex];
    const settings = gameData.gameSettings; // New: Access gameSettings
    ...
    // Reset hover states before checking current frame
    hoveredButton = null;
    hoveredBackButton = false; // New: Reset back button hover state

    // Check for choice button hover
    ...
    // New: Check for back button hover
    if (sceneHistory.length > 0) { // Only enable if there's history
        if (mouseX >= settings.backButtonX && mouseX <= settings.backButtonX + settings.backButtonWidth &&
            mouseY >= settings.backButtonY && mouseY <= settings.backButtonY + settings.backButtonHeight) {
...
function renderPresentationScene() {
    ...
    // Render "Click to proceed" prompt
    ...
    // New: Render back button if there is history to go back to
    if (sceneHistory.length > 0) {
        renderButton(settings.backButtonLabel, settings.backButtonX, settings.backButtonY, settings.backButtonWidth, settings.backButtonHeight, hoveredBackButton);
    }
}
