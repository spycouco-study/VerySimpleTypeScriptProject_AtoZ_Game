...
let sceneHistory: string[] = []; // Stack to store previous scene IDs
let hoveredBackButton: boolean = false;

const BACK_BUTTON_LABEL = "<< 뒤로가기";
const BACK_BUTTON_MARGIN_X = 80;
const BACK_BUTTON_MARGIN_Y = 70;
const BACK_BUTTON_WIDTH = 150;
...
async function init() {
    ...
    canvas.addEventListener('click', () => {
        ...
        if (currentGameState === GameState.TITLE) {
            ...
            playSFX("click_sfx");
            resetSceneState();
            // No history to push as we are just starting
        } else if (currentGameState === GameState.PRESENTATION) {
            // Check for back button click first
            if (hoveredBackButton && sceneHistory.length > 0) {
                playSFX("click_sfx");
                const prevSceneId = sceneHistory.pop()!; // Get the ID of the previous scene
                const prevScene = gameData.scenes.find(s => s.id === prevSceneId);
                if (prevScene) {
                    currentSceneIndex = gameData.scenes.indexOf(prevScene);
                    resetSceneState();
                } else {
                    console.error("Previous scene not found in history.");
                    sceneHistory = []; // Clear history to prevent further issues
                }
                return; // Consume the click event for the back button
            }

            // If not back button, handle regular presentation click
...
function handlePresentationClick() {
    ...
    if (currentScene.choices && hoveredButton) {
        playSFX("click_sfx");
        sceneHistory.push(currentScene.id); // Save current scene to history before moving forward
    ...
    // If text is still typing, advance to full line
    ...
    // If all text is displayed for the current line and no choices were clicked (or there are no choices)
    if (!currentScene.choices || currentScene.choices.length === 0) {
        if (displayedTextIndex < totalLines - 1) { // Move to next line of text
            displayedTextIndex++;
            currentLineCharIndex = 0;
            typingTimer = 0;
            playSFX("click_sfx");
        } else { // All text displayed, move to next scene (if next_scene is defined) or end game
...
function resetSceneState() {
    ...
    typingTimer = 0;
    hoveredButton = null;
    hoveredBackButton = false; // Reset hovered state for back button too
    lastTypeSoundTime = 0;
}
...
function updatePresentation(deltaTime: number) {
    ...
    // Update hovered back button
    hoveredBackButton = false;
    if (sceneHistory.length > 0) { // Only allow hovering if there's history
        if (mouseX >= BACK_BUTTON_MARGIN_X && mouseX <= BACK_BUTTON_MARGIN_X + BACK_BUTTON_WIDTH &&
            mouseY >= BACK_BUTTON_MARGIN_Y && mouseY <= BACK_BUTTON_MARGIN_Y + BACK_BUTTON_HEIGHT) {
...
function renderPresentationScene() {
    ...
    // Render back button if there is history
    if (sceneHistory.length > 0) {
        renderButton(BACK_BUTTON_LABEL, BACK_BUTTON_MARGIN_X, BACK_BUTTON_MARGIN_Y, BACK_BUTTON_WIDTH, BACK_BUTTON_HEIGHT, hoveredBackButton);
    }
