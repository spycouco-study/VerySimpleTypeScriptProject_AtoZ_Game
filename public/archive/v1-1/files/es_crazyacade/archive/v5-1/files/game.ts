...
class Player implements GameEntity {
    ...
    update(deltaTime: number, game: Game): void {
        ...
        const snapThreshold = playerSettings.snapThreshold * settings.tileSize; // How far off center to trigger a snap
        ...
        const playerGridX = Math.floor(playerBoxCenterX / settings.tileSize);
        const playerGridY = Math.floor(playerBoxCenterY / settings.tileSize);

        // Target position if player were perfectly centered in their current grid cell, adjusted for player sprite size
        const targetCenteredX = playerGridX * settings.tileSize + (settings.tileSize - this.width) / 2;
        const targetCenteredY = playerGridY * settings.tileSize + (settings.tileSize - this.height) / 2;
        ...
        if (inputDx !== 0) { // Player is actively trying to move horizontally
            ...
            if (Math.abs(diffX) > settings.tileSize * 0.01) { // Apply snap only if not already almost perfectly centered (to avoid jitter)
        ...
        if (inputDy !== 0) { // Player is actively trying to move vertically
            ...
            if (Math.abs(diffY) > settings.tileSize * 0.01) {
        ...
        if (finalMoveX !== 0) {
            ...
            if (!this.collidesWithWalls(testRectX, game)) {
                newX = this.x + finalMoveX;
            } else {
                // If collision, align player to the edge of the blocking tile
                if (finalMoveX > 0) { // Moving right
                    newX = Math.floor((this.x + this.width) / settings.tileSize) * settings.tileSize - this.width - 0.01;
                } else { // Moving left
                    newX = Math.ceil(this.x / settings.tileSize) * settings.tileSize + 0.01;
                }
            }
        }

        // Attempt Y movement using the potentially updated X position
        if (finalMoveY !== 0) {
            const testRectY = { x: newX, y: this.y + finalMoveY, width: this.width, height: this.height };
            if (!this.collidesWithWalls(testRectY, game)) {
                newY = this.y + finalMoveY;
            } else {
                // If collision, align player to the edge of the blocking tile
                if (finalMoveY > 0) { // Moving down
                    newY = Math.floor((this.y + this.height) / settings.tileSize) * settings.tileSize - this.height - 0.01;
                } else { // Moving up
                    newY = Math.ceil(this.y / settings.tileSize) * settings.tileSize + 0.01;
                }
            }
        }
