...
class Player implements GameEntity {
    ...
    update(deltaTime: number, game: Game): void {
        ...
        if (finalMoveX !== 0) {
            // Pass 'this' player instance to collidesWithWalls for bomb collision logic
    ...
    private getMaxAllowedMove(currentPos: number, targetPos: number,
        ...
        for (let dist = 1; dist <= Math.abs(moveAmount); dist++) {
            ...
            // Pass `this` (the player instance) to allow specific bomb collision rules.
            if (this.collidesWithWalls(testRect, game, this)) {
    ...
    private collidesWithWalls(rect: { x: number, y: number, width: number, height: number }, game: Game, playerRef: Player): boolean {
        const settings = game.getGameSettings();
        const tileSize = settings.tileSize;
        ...
        // Calculate the player's *current origin* grid tile (before any test move represented by `rect`)
        // This is crucial for distinguishing moving *off* a bomb from moving *into* a bomb.
        const playerOriginGridX = Math.floor((playerRef.x + playerRef.width / 2) / tileSize);
        const playerOriginGridY = Math.floor((playerRef.y + playerRef.height / 2) / tileSize);
        ...
        for (const bomb of game.getBombs()) {
            ...
            if (checkCollision(rect, bomb)) {
                ...
                // If the player's *original current position* (before attempting to move)
                ...
                // We use the player's original position, not the `rect` (test position), to identify this.
                if (playerOriginGridX === bombGridX && playerOriginGridY === bombGridY) {
