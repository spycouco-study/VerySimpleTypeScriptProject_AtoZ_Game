...
class Player implements GameEntity {
    ...
    update(deltaTime: number, game: Game): void {
        ...
        // --- Capture player's grid position at the start of the frame for consistent bomb collision logic ---
        const initialPlayerCenterX = this.x + this.width / 2;
        const initialPlayerCenterY = this.y + this.height / 2;
        const originalPlayerGridX = Math.floor(initialPlayerCenterX / tileSize);
        const originalPlayerGridY = Math.floor(initialPlayerCenterY / tileSize);
        // --- End capture ---
        ...
        if (finalMoveX !== 0) {
            // Pass original player grid position for bomb collision logic
            const actualMoveX = this.getMaxAllowedMove(this.x, this.x + finalMoveX, this.y, this.width, this.height, true, game, originalPlayerGridX, originalPlayerGridY);
            this.x += actualMoveX;
        }

        if (finalMoveY !== 0) {
            // Pass original player grid position for bomb collision logic
            const actualMoveY = this.getMaxAllowedMove(this.y, this.y + finalMoveY, this.x, this.width, this.height, false, game, originalPlayerGridX, originalPlayerGridY);
            this.y += actualMoveY;
        }
    }

    // Helper method: Determines the maximum distance the player can move in a given direction
    // without colliding with any solid wall or bomb. It checks pixel by pixel for precision.
    private getMaxAllowedMove(currentPos: number, targetPos: number,
                              otherAxisPos: number, entityWidth: number, entityHeight: number,
                              isXAxis: boolean, game: Game,
                              originalPlayerGridX: number, originalPlayerGridY: number): number {
        const desiredMoveAmount = targetPos - currentPos;
        if (desiredMoveAmount === 0) return 0;

        const sign = Math.sign(desiredMoveAmount);
        const totalDistance = Math.abs(desiredMoveAmount);
        let allowedDistance = 0;

        // Iterate in small, fractional steps to find the maximum allowed distance.
        // This addresses issues with integer-only step checks causing players to get stuck
        // when their position is fractional and cannot make a full pixel move.
        const collisionStep = 0.5; // Check every half-pixel for finer collision detection

        for (let dist = collisionStep; dist <= totalDistance; dist += collisionStep) {
            // Ensure we don't test beyond the desired total distance in the last step
            const testDist = Math.min(dist, totalDistance);
            const testMovePos = currentPos + testDist * sign;
            ...
            if (this.collidesWithWalls(testRect, game, originalPlayerGridX, originalPlayerGridY)) {
                // Collision detected. The maximum safe distance is the 'allowedDistance'
                // accumulated from previous steps (before this collision).
                return allowedDistance * sign; // Return with the correct sign
            } else {
                // No collision, this distance is safe. Store it as the current maximum allowed.
                allowedDistance = testDist;
            }
        }
        // If the loop completes, no collision was found up to the totalDesiredDistance.
    ...
    // or bombs. It allows the player to move off a bomb they are currently occupying.
    private collidesWithWalls(rect: { x: number, y: number, width: number, height: number }, game: Game,
                              originalPlayerGridX: number, originalPlayerGridY: number): boolean { // Modified signature
        const settings = game.getGameSettings();
        const tileSize = settings.tileSize;
        ...
        // 1. Check collision with solid map tiles
        ...
        // 2. Check collision with bombs
        // Use the `originalPlayerGridX` and `originalPlayerGridY` passed to the function.
        // These represent the player's grid position at the start of the current update frame,
        // ensuring consistent bomb-escape logic even if player's coordinates change mid-frame.
        
        for (const bomb of game.getBombs()) {
            // Bombs are square and tile-aligned, so their bounding box is effectively a tile.
            // Check if the proposed `rect` (potential new player position) collides with the bomb's rectangle.
            if (checkCollision(rect, bomb)) {
                // Calculate the bomb's grid tile. Bombs are always tile-aligned.
                const bombGridX = Math.floor(bomb.x / tileSize);
                const bombGridY = Math.floor(bomb.y / tileSize);

                // If the player's *original current position* (at the start of the frame)
                // is on the same tile as this bomb, it means the player just placed this bomb
                // and is attempting to move *off* of it. This movement should be allowed.
                if (originalPlayerGridX === bombGridX && originalPlayerGridY === bombGridY) {
                    continue; // Player can move off this bomb, so ignore this collision.
                }

                // Otherwise, the `rect` (potential new player position) is colliding with a bomb
                // that the player is *not* currently occupying (either another bomb or their own
                // bomb they already moved away from). This should block movement.
