// 캔버스 설정
const canvas2 = document.getElementById('gameCanvas') as HTMLCanvasElement;
...
function preloadAssets(): void {
    ...
    (window as any).appleImage = snakeImages.apple;
    assetsLoaded = true;
}
...
function drawSnake(): void {
    if (!ctx2 || !snakeImages || snake.length === 0) return;
    // Head (진행 방향을 따름)
    const head = snake[0];
