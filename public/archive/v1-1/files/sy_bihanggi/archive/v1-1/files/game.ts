interface GameSettings {
    canvasWidth: number;
    canvasHeight: number;
    scrollSpeed: number; // Pixels per second for overall scrolling effect
    playerSpeed: number;
    playerFireRate: number; // bullets per second
    ...
    enemySpeed: number;
    enemyHealth: number;
    enemyScoreValue: number;
    enemyBulletSpeed: number;
    enemyFireRate: number; // enemy bullets per second
    enemyFireChance: number; // chance for an enemy to fire when spawned (per second, roughly)
    maxEnemies: number;
    levelDuration: number; // seconds until game ends (e.g., victory)
    backgroundSpeedMultiplier: number; // how much faster background scrolls than base scrollSpeed
}

interface ImageAssetConfig {
    name: string;
    path: string;
    width: number; // Target width for drawing
    height: number; // Target height for drawing
}
...
interface SoundAssetConfig {
    ...
    path: string;
    volume: number;
    loop?: boolean; // Optional: for background music
}
...
class Player implements GameObject {
    ...
    constructor(x: number, y: number, width: number, height: number, speed: number, imageName: string, health: number) {
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
        ...
        if ((inputState[' '] || inputState['space']) && this.fireCooldown <= 0) {
            ...
            if (bulletImg) {
                playerBullets.push(new Bullet(
                    this.x + this.width / 2 - bulletImg.width / 2,
                    this.y,
                    bulletImg.width,
                    bulletImg.height,
                    -config.game.playerBulletSpeed, // Y velocity (upwards)
                    'player_bullet',
                    config.game.playerBulletDamage,
                    'player'
                ));
...
class Bullet implements GameObject {
    ...
    width: number;
    height: number;
    speed: number; // Y velocity (positive for down, negative for up)
    imageName: string;
    damage: number;
    owner: 'player' | 'enemy';

    constructor(x: number, y: number, width: number, height: number, speed: number, imageName: string, damage: number, owner: 'player' | 'enemy') {
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
...
class Enemy implements GameObject {
    ...
    width: number;
    height: number;
    speed: number; // Y velocity (downwards)
    imageName: string;
    health: number;
    fireCooldown: number = 0;
        ...
        if (this.fireCooldown <= 0 && Math.random() < config.game.enemyFireChance * dt) {
            ...
            if (bulletImg) {
                 enemyBullets.push(new Bullet(
                    ...
                    bulletImg.width,
                    bulletImg.height,
                    config.game.enemyBulletSpeed, // Y velocity (downwards)
                    'enemy_bullet',
                    1, // Enemy bullets deal 1 damage by default
                    'enemy'
                ));
...
function stopBGM(name: string): void {
    const soundAsset = loadedSounds.get(name);
...
async function loadAssets(config: GameConfig): Promise<void> {
...
function startGame(): void {
    ...
    player = new Player(
        ...
        canvas.height - playerImg.height - 30,
        playerImg.width,
        playerImg.height,
        config.game.playerSpeed,
        'player',
        config.game.playerHealth
    );
...
function update(dt: number): void {
    switch (currentGameState) {
        case GameState.PLAYING:
            ...
            if (enemySpawnTimer >= config.game.enemySpawnInterval && enemies.length < config.game.maxEnemies) {
                ...
                if (enemyImageConfig) {
                    ...
                    enemies.push(new Enemy(
                        x, y,
                        enemyImageConfig.width, enemyImageConfig.height,
                        config.game.enemySpeed,
                        'enemy',
                        config.game.enemyHealth
                    ));
                    enemySpawnTimer = 0;
                }
            }
            ...
            for (let i = playerBullets.length - 1; i >= 0; i--) {
                const bullet = playerBullets[i];
                if (bullet.owner !== 'player') continue; // Only player bullets hit enemies
            ...
            for (let i = enemyBullets.length - 1; i >= 0; i--) {
                const bullet = enemyBullets[i];
                if (bullet.owner !== 'enemy') continue; // Only enemy bullets hit player
