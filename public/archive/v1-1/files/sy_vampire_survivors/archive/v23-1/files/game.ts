...
function drawSprite(assetName: string, x: number, y: number, width: number, height: number, applyShadow: boolean = true): void {
    const image = assets.images.get(assetName);
    if (image) {
        // Calculate the top-left drawing position relative to the camera
        const drawX = x - width / 2 - cameraX;
        const drawY = y - height / 2 - cameraY;
