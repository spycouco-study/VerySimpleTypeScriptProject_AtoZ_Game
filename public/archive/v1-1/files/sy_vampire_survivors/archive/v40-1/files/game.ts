...
interface GameData {
    ...
    experienceGems: {
        ...
        lifetime: number; // NEW: Time until the gem disappears
        blinkThreshold: number; // NEW: Time remaining when the gem starts blinking
        blinkInterval: number; // NEW: The interval at which the gem blinks
    };
    gameplay: {
...
function dropExperienceGem(x: number, y: number, value: number): void {
    experienceGems.push({
        ...
        lifetime: gameData.experienceGems.lifetime, // Initialize lifetime for new gems
    });
}
...
function drawGameplay(): void {
    ...
    for (const gem of experienceGems) {
        let shouldDraw = true;
        // Apply blinking effect if lifetime is below threshold
        if (gem.lifetime <= gameData.experienceGems.blinkThreshold) {
            // Calculate if the gem should be visible this frame for blinking
            shouldDraw = Math.floor(gameTimer / gameData.experienceGems.blinkInterval) % 2 === 0;
        }
