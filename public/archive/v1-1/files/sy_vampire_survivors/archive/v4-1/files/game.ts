...
interface GameData {
    ...
    gameplay: {
        ...
        attackSpeedIncreasePerLevel: number; // NEW: Percentage reduction in attack cooldown per level (e.g., 0.02 for 2%)
    };
    ui: {
        font: string;
        textColor: string;
...
function startNewGame(): void {
    ...
    for (let i = 0; i < gameData.gameplay.initialEnemyCount; i++) {
        spawnSingleEnemy(true); // Spawn initially within bounds or slightly off
    }
...
function updatePlayer(dt: number): void {
    ...
    if (player.currentAttackCooldown <= 0) {
        ...
        if (targetEnemy) {
            fireProjectile(player, targetEnemy);
...
function fireProjectile(source: Player, target: Enemy): void {
    const projSpeed = gameData.player.projectileSpeed;
    const dx = target.x - source.x;
    const dy = target.y - source.y;
    const dist = Math.sqrt(dx * dx + dy * dy);

    if (dist === 0) return; // Prevent division by zero if target is exactly on source
...
function updateProjectiles(dt: number): void {
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        // Remove if lifetime expires or goes off-screen
...
function spawnEnemies(dt: number): void {
    ...
    if (lastEnemySpawnTime >= gameData.gameplay.enemySpawnInterval && enemies.length < gameData.gameplay.maxEnemies) {
        lastEnemySpawnTime = 0;
        spawnSingleEnemy(false); // Spawn continuously off-screen
    }
}
...
function spawnSingleEnemy(initialSpawn: boolean): void {
    ...
    if (initialSpawn) {
        // For initial enemies, spawn them somewhat centered on the screen
        x = Math.random() * (canvas.width - enemyTypeConfig.size) + enemyTypeConfig.size / 2;
        y = Math.random() * (canvas.height - enemyTypeConfig.size) + enemyTypeConfig.size / 2;
    } else {
        // For continuous spawns, spawn completely off-screen
...
function isWithinBounds(x: number, y: number, size: number): boolean {
    return x + size / 2 > 0 && x - size / 2 < canvas.width &&
           y + size / 2 > 0 && y - size / 2 < canvas.height;
}
...
function drawGameplay(): void {
    ...
    for (const enemy of enemies) {
        ...
        // Optional: Draw enemy health bar
        // drawHealthBar(enemy.x, enemy.y - enemy.size / 2 - 10, enemy.size, 5, enemy.health, enemy.maxHealth, 'red', 'darkred');
    }
