...
// Define a slippery material for all objects
...
const slipperyContactMaterial = new CANNON.ContactMaterial(
  ...
  slipperyMaterial,
  {
    ...
    restitution: 0.1, // Slightly bouncy, feel free to adjust
  }
);
...
const groundBody = new CANNON.Body({
  ...
  material: slipperyMaterial, // Assign slippery material
});
...
playerBody.addEventListener("collide", (event: any) => {
  ...
  if (event.body === groundBody || obstacles.some(o => o.body === event.body)) {
    // Only allow jump if the collision normal suggests landing on top
    // For more robust checking, can inspect event.contact.ni (normal vector)
    const contactNormal = new CANNON.Vec3();
    event.contact.ni.negate(contactNormal); // Get normal relative to player
    if(contactNormal.y > 0.5) { // If player hit something from above
      canJump = true;
    }
  }
});
...
// In a real scenario, this would likely be part of the canvas UI or a separate overlay.
...
function updatePlayer() {
  ...
  // Apply velocity directly for a more direct control, or apply force for more physical interaction
  // With lower friction, direct velocity setting feels more responsive.
