...
class AssetLoader {
    constructor(assetConfig) {
        this.images = new Map();
        this.audioBGM = new Map(); // For BGM (looping sounds)
        this.audioEffects = new Map(); // For effects (non-looping, with pooling)
...
class Game {
    constructor(canvasId) {
        ...
        this.newDirection = { x: 0, y: 0 }; // Next movement direction (buffered)
        this.score = 0;
        this.gamePaused = false; // To prevent immediate restart after Game Over
    ...
    handleKeyDown(event) {
        ...
        if (this.gamePaused && keyCode !== 32)
            return; // Ignore input if paused, unless it's space
        switch (this.currentGameState) {
            ...
            case Playing:
                ...
                if (keyCode === 37 && this.direction.x === 0)
                    newDir = { x: -1, y: 0 }; // Left
                else if (keyCode === 38 && this.direction.y === 0)
                    newDir = { x: 0, y: -1 }; // Up
                else if (keyCode === 39 && this.direction.x === 0)
                    newDir = { x: 1, y: 0 }; // Right
                else if (keyCode === 40 && this.direction.y === 0)
                    newDir = { x: 0, y: 1 }; // Down
    ...
    draw() {
        ...
        switch (this.currentGameState) {
            case GameState.Title:
                ...
                if (titleBgImage) {
                    ctx.drawImage(titleBgImage, 0, 0, game.canvasWidth, game.canvasHeight);
                }
                else {
            ...
            case GameState.Playing:
                ...
                if (this.food) {
                    ...
                    if (foodImage) {
                        ctx.drawImage(foodImage, this.food.x * game.tileSize, this.food.y * game.tileSize, game.tileSize, game.tileSize);
                    }
                    else {
                ...
                this.snake.forEach((segment, index) => {
                    ...
                    if (segmentImage) {
                        ctx.drawImage(segmentImage, segment.x * game.tileSize, segment.y * game.tileSize, game.tileSize, game.tileSize);
                    }
                    else {
