...
class BubbleMatchGame {
    ...
    constructor(canvasId) {
        this.GRID_SIZE = 7; // 그리드 크기 (4x4에서 7x7으로 변경)
        this.COLORS = ['red', 'blue', 'green', 'purple', 'orange', 'yellow']; // 사용 가능한 6가지 색상
        this.EMPTY_CELL = -1; // 빈 셀을 나타내는 값
        this.score = 0; // 현재 점수
        this.selectedBubble = null; // 선택된 방울의 좌표
        this.isProcessing = false; // 매치 처리, 방울 낙하, 리필 등의 작업 중인지 여부 (중복 클릭 방지)
        this.isGameOver = false; // 게임 오버 상태인지 여부 (새로 추가됨)
        this.gameOverReason = ''; // 게임 오버 원인을 저장할 새 속성
        this.gameStarted = false; // 게임 시작 여부를 나타내는 새 속성
        // --- 새로운 상태바 및 시간 제한 관련 속성 ---
        this.statusBarHeight = 40; // 상태바의 높이 (픽셀)
        this.gameDuration = 60; // 게임 시간 제한 (초) - 1분으로 변경 (이전 120초)
        this.timerInterval = null; // setInterval ID를 저장할 변
        // --- 애셋 관련 새 속성 ---
        this.spriteSheet = null;
        this.SPRITE_SHEET_PATH = 'animals.png';
        this.SPRITE_WIDTH = 250; // animals.png 내 개별 스프라이트의 가로 크기 (가정) - **수정됨**
        this.SPRITE_HEIGHT = 250; // animals.png 내 개별 스프라이트의 세로 크기 (가정) - **수정됨**
        this.spriteMap = []; // 각 colorIndex에 해당하는 스프라이트 시트 내 시작 X, Y 좌표
        this.audioBGM = null;
        this.audioEat = null; // <-- HTMLAUdioElement 오타 수정
        this.BGM_PATH = 'bgm.mp3';
        this.EAT_SFX_PATH = 'pop.mp3'; // assets\pop.mp3로 경로 수정
        // --- 배경 이미지 관련 새 속성 ---
        this.backgroundImage = null; // 배경 이미지를 위한 새 속성
        this.BACKGROUND_IMAGE_PATH = 'back.png'; // 배경 이미지 경로
        // --- 애셋 관련 끝 ---
        // --- 애니메이션 관련 새 속성 ---
        this.animationQueue = []; // BubbleMovement 타입 적용
        this.animationStartTime = null;
        this.animationDuration = 250; // 방울 이동 애니메이션 지속 시간 (ms)
        this.isAnimatingMovement = false; // 방울 이동 애니메이션 중인지 여부
        // 변경: animatingPops에 startTime 속성 추가
        this.animatingPops = [];
        // private popStartTime: number | null = null; // 이제 각 방울이 개별 startTime을 가질 것이므로 필요 없음
        this.popDuration = 200; // 방울 터지는 애니메이션 지속 시간 (ms)
        // 새로 추가된 방울 터짐 효과 스프라이트 관련 속성
        this.popEffectSpriteSheet = null;
        this.POP_EFFECT_SPRITE_PATH = 'bubble_effect.png';
        this.POP_EFFECT_FRAME_HEIGHT = 305;
        this.POP_EFFECT_NUM_FRAMES = 5; // 1535px 너비에 5개 프레임 가정
     ...
     */
    hasMatchAt(r, c, targetGrid = this.grid) {
        const color = targetGrid[r][c];
        if (color === this.EMPTY_CELL)
            return false;
        // 수평 매치 확인 (왼쪽 두 칸)
        if (c > 1 && targetGrid[r][c - 1] === color && targetGrid[r][c - 2] === color)
            return true;
        // 수직 매치 확인 (위쪽 두 칸)
        if (r > 1 && targetGrid[r - 1][c] === color && targetGrid[r - 2][c] === color)
            return true;
        return false;
    }
    /**
    ...
    draw(movementProgress = 1) {
        ...
        const movingTargetCoords = new Set();
        ...
        for (let r = 0; r < this.GRID_SIZE; r++) {
            for (let c = 0; c < this.GRID_SIZE; c++) {
                ...
                if (bubbleColorIndex !== this.EMPTY_CELL && !movingTargetCoords.has(`${r},${c}`)) {
                    ...
                    const isSelected = !!(this.selectedBubble && this.selectedBubble.row === r && this.selectedBubble.col === c);
     ...
     */
    drawBubble(colorIndex, x, y, isSelected) {
        if (colorIndex === this.EMPTY_CELL)
            return;
        ...
        if (this.spriteSheet && this.spriteMap[colorIndex]) {
            ...
            this.ctx.drawImage(this.spriteSheet, spriteInfo.sx, spriteInfo.sy, this.SPRITE_WIDTH, this.SPRITE_HEIGHT, x + offsetX, y + offsetY, drawSize, drawSize);
        }
        else {
    ...
    drawBubbleFade(colorIndex, x, y, progress) {
        ...
        if (this.spriteSheet && this.spriteMap[colorIndex]) {
            ...
            this.ctx.drawImage(this.spriteSheet, spriteInfo.sx, spriteInfo.sy, this.SPRITE_WIDTH, this.SPRITE_HEIGHT, x + offsetX, y + offsetY, currentDrawSize, currentDrawSize);
        }
        else {
            // 스프라이트 로딩에 실패했거나 정보가 없는 경우, 기존처럼 색상 원형 방울을 그립니다.
            ...
            this.ctx.fillStyle = this.COLORS[colorIndex];
            this.ctx.beginPath();
            this.ctx.arc(x + this.CELL_SIZE / 2, y + this.CELL_SIZE / 2, radius, 0, Math.PI * 2);
            this.ctx.fill();
            this.ctx.strokeStyle = 'black';
            this.ctx.lineWidth = 1;
            this.ctx.stroke();
     ...
     */
    drawPopEffectSprite(x, y, progress) {
        // 효과 스프라이트 시트가 로드되지 않았거나 애니메이션이 완료되면 그리지 않음
        if (!this.popEffectSpriteSheet || progress >= 1)
            return;
        // 현재 애니메이션 프레임 인덱스 계산
        const currentFrameIndex = Math.min(this.POP_EFFECT_NUM_FRAMES - 1, Math.floor(progress * this.POP_EFFECT_NUM_FRAMES));
        ...
        this.ctx.drawImage(this.popEffectSpriteSheet, sx, sy, this.POP_EFFECT_FRAME_WIDTH, this.POP_EFFECT_FRAME_HEIGHT, x + offsetX, y + offsetY, currentEffectSize, currentEffectSize);
        this.ctx.restore();
    }
    /**
     ...
     */
    async _startInitialMatchProcessing() {
     ...
     */
    async trySwapAndMatch(r1, c1, r2, c2) {
     ...
     * @param targetGrid (선택 사항) 매치 확인에 사용할 그리드 (기본값: this.grid)
     */
    findMatches(targetGrid = this.grid) {
     ...
     */
    calculateGravityAndRefillChanges() {
        const movements = []; // BubbleMovement 타입 적용
    ...
    animatePop(removedBubbles) {
        ...
        if (this.audioEat) {
            ...
            this.audioEat.play().catch((e) => console.warn("효과음 재생 실패:", e)); // <-- 'e' 매개변수 타입 지정
        }
    ...
    startTimer() {
        ...
        }, 1000); // setInterval의 반환 값 타입 추론 오류 방지
    }
    /**
     * (새로 추가됨)
    ...
    canMakeMove() {
        ...
        for (let r = 0; r < this.GRID_SIZE; r++) {
            for (let c = 0; c < this.GRID_SIZE; c++) {
                ...
                if (currentColor === this.EMPTY_CELL)
                    continue;
