...
interface GameConfig {
  player: {
    speed: number;
    sprintSpeedMultiplier: number; // Added sprint speed multiplier
    jumpForce: number;
...
let moveRight = false;
let canJump = true;
let isSprinting = false; // New: Tracks if player is sprinting
  ...
  const onKeyUp = (event: KeyboardEvent) => {
    switch (event.code) {
      ...
      case "KeyD":
        moveRight = false;
        break;
      case "ShiftLeft": // New: Stop Sprint
        isSprinting = false;
        break;
    }
  };
...
function animate() {
  ...
  // 2. 플레이어 움직임 처리
  // Calculate current effective speed based on sprint state
  ...
  const speed = currentMoveSpeed * dt; // Apply delta time to get distance per frame

  const bodyVelocity = playerBody.velocity;
  const currentVelocity = new THREE.Vector3(0, bodyVelocity.y, 0); // Preserve current Y velocity (gravity/jump)

  // 카메라 방향을 기준으로 움직임 벡터 계산
  const direction = new THREE.Vector3();
  camera.getWorldDirection(direction);
  direction.y = 0; // Y축(수직) 움직임은 무시
  direction.normalize(); // Normalize after setting y to 0 to maintain consistent speed
