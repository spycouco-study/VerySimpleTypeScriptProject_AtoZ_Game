...
class Game {
    ...
    private spawnCollectible(associatedObstacle?: Obstacle, customX?: number): void {
        ...
        if (associatedObstacle) {
            // Collectible with an obstacle: Position it such that the player can jump to acquire it.
            collectibleX = associatedObstacle.x + associatedObstacle.width / 2 - collectible.width / 2;

            const obstacleTopY = associatedObstacle.y; // Top edge of the obstacle
            const playerBaseY = this.player.baseY; // Player's top Y when running on ground
            const playerJumpPeakY = playerBaseY - this.config.player.jumpHeight; // Highest Y player can reach during a jump

            // 1. Calculate a target Y relative to the obstacle, ensuring clearance above it.
            const yFromObstacleTop = obstacleTopY - collectible.height - 20; // 20px buffer above obstacle

            // 2. Calculate a target Y that is generally within a comfortable jump range (e.g., mid-jump).
            const jumpableYFromGroundMinOffset = this.config.player.height * 0.5; // At least half player height above ground
            const jumpableYFromGroundMaxOffset = this.config.player.jumpHeight * 0.8; // Up to 80% of max jump height
            ...
            // Combine these: choose the higher (smaller Y value) to ensure it's both above the obstacle AND within a general jumpable range
            collectibleY = Math.min(yFromObstacleTop, randomJumpableYFromGround);

            // 3. Apply final clamps to ensure playability:
            // Must be above player's running head (so it requires a jump)
            collectibleY = Math.min(collectibleY, playerBaseY - collectible.height - 10); // 10px buffer above player's top when running
            // Must be reachable by player's max jump (cannot be higher than peak)
            collectibleY = Math.max(collectibleY, playerJumpPeakY);
            // Must be strictly above the obstacle (re-clamp with minimum buffer)
            collectibleY = Math.min(collectibleY, obstacleTopY - collectible.height - 10);

            // Add slight randomness around the final calculated Y
            collectibleY += (Math.random() - 0.5) * 10; // +/- 5px

        } else {
            // Standalone collectible: Position randomly in the air, not tied to an obstacle.
            collectibleX = customX !== undefined ? customX : this.canvas.width + Math.random() * 100;

            // Use the config's collectibleSpawnOffset for random air position from ground
            const offsetFromGround = this.config.gameplay.collectibleSpawnOffsetMin +
            ...
            // Ensure it's not too low (on ground) or too high (unreachable)
            collectibleY = Math.min(collectibleY, this.config.ground.y - collectible.height - 10); // At least 10px off ground
            collectibleY = Math.max(collectibleY, this.player.baseY - this.config.player.jumpHeight * 1.1); // Max 110% of player jump height
        }
