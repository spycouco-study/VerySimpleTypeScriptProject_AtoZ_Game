...
interface PlayerConfig {
    ...
    lives: number;
    maxJumps: number; // Number of jumps allowed (1 for single, 2 for double)
}
...
interface GameplayConfig {
    ...
    collectibleSpawnIntervalMin: number; // Min time before next standalone collectible spawn (NEW)
    collectibleSpawnIntervalMax: number; // Max time before next standalone collectible spawn (NEW)
    collectibleSpawnChance: number; // Probability to spawn a collectible with an obstacle
    collectibleSpawnOffsetMin: number; // Y offset from ground for standalone collectible
    collectibleSpawnOffsetMax: number; // Y offset from ground for standalone collectible
...
class Player {
    ...
    blinkTimer: number = 0;
    lives: number;
    jumpCount: number = 0; // Tracks consecutive jumps
    ...
    jump(): boolean {
        // Allow jump if on ground OR if has remaining jumps (for double jump)
        if (this.onGround && this.animationState !== PlayerAnimationState.SLIDING) {
            ...
            return true;
        } else if (!this.onGround && this.jumpCount < this.config.maxJumps && this.animationState !== PlayerAnimationState.SLIDING) {
            // Double jump (or multiple jumps based on maxJumps)
            this.velocityY = -this.config.jumpForce; // Apply full jump force again
            this.animationState = PlayerAnimationState.JUMPING;
...
class Game {
    ...
    private collectibleSpawnTimer: number = 0; // NEW: Timer for standalone collectibles
    private nextCollectibleSpawnTime: number = 0; // NEW: Next spawn time for standalone collectibles
    ...
    private resetSpawnTimers() {
        ...
        // NEW: Initialize collectible spawn timer
    ...
    private update(deltaTime: number): void {
        switch (this.state) {
            case GameState.PLAYING:
                ...
                // Spawn obstacles and their associated collectibles
                this.obstacleSpawnTimer += deltaTime;
                if (this.obstacleSpawnTimer >= this.nextObstacleSpawnTime) {
                    this.spawnObstacleAndLinkedCollectible(); // Modified to handle linked collectibles
                    this.obstacleSpawnTimer = 0;
                    this.nextObstacleSpawnTime = this.getRandomSpawnTime(this.config.gameplay.obstacleSpawnIntervalMin, this.config.gameplay.obstacleSpawnIntervalMax);
                }

                // NEW: Spawn standalone collectibles
                this.collectibleSpawnTimer += deltaTime;
                if (this.collectibleSpawnTimer >= this.nextCollectibleSpawnTime) {
                    this.spawnStandaloneCollectible(); // New function for collectibles without obstacles
                    this.collectibleSpawnTimer = 0;
                    this.nextCollectibleSpawnTime = this.getRandomSpawnTime(
                        this.config.gameplay.collectibleSpawnIntervalMin,
                        this.config.gameplay.collectibleSpawnIntervalMax
    ...
    private spawnObstacleAndLinkedCollectible(): void {
        ...
        const spawnX = this.canvas.width + Math.random() * 100; // Spawn slightly off-screen to the right
        ...
        // Potentially spawn a collectible *with* this obstacle, at a jumpable position
    ...
    private spawnStandaloneCollectible(): void {
        const spawnX = this.canvas.width + Math.random() * 150; // Spawn further ahead for standalone
        this.spawnCollectible(undefined, spawnX); // Call with no associated obstacle
    }

    // Modified to accept optional associatedObstacle for positioning logic
    private spawnCollectible(associatedObstacle?: Obstacle, customX?: number): void {
        ...
        let collectibleY: number;
        let collectibleX: number;

        // Player's Y coordinate for the top of their sprite when on the ground
        const playerGroundTopY = this.player.baseY;
        // Player's Y coordinate for the top of their sprite at the peak of a single jump
        const playerSingleJumpApexTopY = playerGroundTopY - this.config.player.jumpHeight;
        // Player's Y coordinate for the bottom of their sprite at the peak of a single jump
        const playerSingleJumpApexBottomY = playerSingleJumpApexTopY + this.player.height;

        if (associatedObstacle) {
            // Collectible associated with an obstacle
            collectibleX = associatedObstacle.x + associatedObstacle.width / 2 - collectible.width / 2;
            const obstacleTopY = associatedObstacle.y;

            // Target collectibleY (top of collectible)
            let targetY: number;

            // 1. Minimum Y (highest position) the collectible should be to ensure reachability.
            // We want the collectible's top to be slightly below the player's head at jump apex for comfortable collection.
            const idealCollectibleTopY_reachable = playerSingleJumpApexTopY + (this.player.height * 0.2); // 20% down from player's head at apex

            // 2. Maximum Y (lowest position) the collectible should be while clearing the obstacle.
            // It should be at least 10px above the obstacle's top.
            const maxCollectibleTopY_clearance = obstacleTopY - collectible.height - 10;
            
            // Choose the Y position. We start with the ideal reachable Y.
            targetY = idealCollectibleTopY_reachable;

            // If placing it at `idealCollectibleTopY_reachable` causes it to overlap or be too close to the obstacle,
            // we must raise it (make Y value smaller) to clear the obstacle.
            // Check if collectible bottom (targetY + collectible.height) is below obstacle top (obstacleTopY) with a buffer.
            if (targetY + collectible.height + 5 > obstacleTopY) { // 5px buffer for collision
                targetY = maxCollectibleTopY_clearance;
            }

            collectibleY = targetY;
            collectibleY += (Math.random() - 0.5) * 20; // Add slight randomness +/- 10px

        } else {
            // Standalone collectible
            collectibleX = customX !== undefined ? customX : this.canvas.width + Math.random() * 100;

            // Use gameplay config's offsets for standalone collectibles, relative to ground
            let tempY = this.config.ground.y - (this.config.gameplay.collectibleSpawnOffsetMin +
                                     Math.random() * (this.config.gameplay.collectibleSpawnOffsetMax - this.config.gameplay.collectibleSpawnOffsetMin));

            // Ensure it's not too high (unreachable by single jump)
            // The collectible's top should not be higher (smaller Y value) than player's top at peak jump + a small buffer.
            const maxAllowedCollectibleTopY = playerSingleJumpApexTopY + (this.player.height * 0.2);
            tempY = Math.max(tempY, maxAllowedCollectibleTopY); // Ensures collectibleY is not smaller (higher) than this.

            // Ensure it's not too low (on the ground)
            // The collectible's bottom should be at least 10px off the ground.
            const minAllowedCollectibleTopY = this.config.ground.y - collectible.height - 10;
            tempY = Math.min(tempY, minAllowedCollectibleTopY); // Ensures collectibleY is not larger (lower) than this.
        ...
        // Final safety clamps
        collectibleY = Math.max(collectibleY, 0); // Cannot go above canvas top (Y=0)
        collectibleY = Math.min(collectibleY, this.config.ground.y - collectible.height - 5); // Cannot go below 5px off ground
