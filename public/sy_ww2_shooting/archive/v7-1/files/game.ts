...
class Game {
    ...
    renderTitleScreen(): void {
        ...
        if (titleImage) {
            // Use drawImageContain for the title background to prevent cropping and distortion
    ...
    renderInstructionsScreen(): void {
        ...
        const titleImage = this.images.get("title_background");
        if (titleImage) {
            // Use drawImageContain for the instructions background to prevent cropping and distortion
            this.drawImageContain(this.ctx, titleImage, this.canvas.width, this.canvas.height);
        } else {
            this.ctx.fillStyle = 'darkblue';
            this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);
