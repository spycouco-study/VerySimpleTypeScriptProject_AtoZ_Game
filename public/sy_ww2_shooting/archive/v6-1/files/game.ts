...
interface EnemyConfig {
    ...
    shootSound: string; // Added shootSound to EnemyConfig
}
    ...
    update(deltaTime: number, game: Game): void {
        ...
        if ((game.input.get('Space') || game.input.get('KeyJ')) && (game.currentTime - this.lastShotTime) > (1000 / this.fireRate)) {
            ...
            game.playerBullets.push(new Bullet(
                ...
                bulletSpeed, // vx (shoots straight right)
                0,           // vy
    ...
    takeDamage(damage: number, game: Game): void {
        if (this.invincibleTimer <= 0) {
            ...
            if (this.health <= 0) {
                ...
                game.playSound("explosion_sound"); // Changed to explosion_sound
...
class Bullet extends GameObject {
    ...
    constructor(x: number, y: number, width: number, height: number, imageName: string, vx: number, vy: number, damage: number, type: "player" | "enemy") {
    ...
    update(deltaTime: number, game: Game): void {
        this.x += this.vx * deltaTime; // Use vx for horizontal movement
        this.y += this.vy * deltaTime; // Use vy for vertical movement

        // Mark for deletion if off screen in any direction
        if (this.x > game.canvas.width || this.x + this.width < 0 ||
            this.y > game.canvas.height || this.y + this.height < 0) {
            this.markedForDeletion = true;
        }
    }
}
...
class Enemy extends GameObject {
    ...
    shootSound: string; // Added shootSound
    ...
    constructor(x: number, y: number, config: EnemyConfig, game: Game) {
        ...
        this.shootSound = config.shootSound; // Initialize shootSound
    }
    ...
    update(deltaTime: number, game: Game): void {
        ...
        // Shooting: Enemies now fire bullets towards the player's position
        if (this.fireRate > 0 && (game.currentTime - this.lastShotTime) > (1000 / this.fireRate) && game.player && !game.player.markedForDeletion) {
            const bulletSpawnX = this.x - this.bulletType.width; // Spawn bullet from enemy's left edge
            const bulletSpawnY = this.y + this.height / 2 - this.bulletType.height / 2; // Centered vertically
            ...
            if (magnitude > 0) {
                // Normalize the direction vector and multiply by bullet speed
                const normalizedDirectionX = directionX / magnitude;
                const normalizedDirectionY = directionY / magnitude;
                bulletVx = normalizedDirectionX * this.bulletType.speed;
                bulletVy = normalizedDirectionY * this.bulletType.speed;
            } else {
                // Fallback: If enemy is on top of player (magnitude is 0), shoot straight left
            ...
            game.enemyBullets.push(new Bullet(
                ...
                this.bulletType.height,
                this.bulletType.image,
                bulletVx, // Calculated velocity X
                bulletVy, // Calculated velocity Y
                this.bulletType.damage,
                "enemy"
            ));
            game.playSound(this.shootSound); // Use the enemy's specific shoot sound
            this.lastShotTime = game.currentTime;
        }
...
class Background {
    ...
    scrollSpeed: number;
    x1: number = 0;
    x2: number = 0; // for continuous scrolling
    gameWidth: number;
    gameHeight: number;
    ...
    constructor(imageName: string, scrollSpeed: number, gameWidth: number, gameHeight: number, game: Game) {
        ...
        if (this.image) {
            // Initialize positions for two tiles to cover the screen and beyond
            this.x1 = 0;
            // Ensure x2 starts where x1 ends, handling potential image width differences
            // The image might not be exactly canvas width, so we tile it based on its own width.
    ...
    update(deltaTime: number): void {
        ...
        // If an image tile moves completely off-screen to the left, reset it to the right
    ...
    draw(ctx: CanvasRenderingContext2D): void {
        if (this.image) {
            // Draw both background tiles, scaled to canvas height
...
class Game {
    ...
    private drawLoadingScreen(message: string): void {
        // Ensure canvas dimensions are set even if data isn't loaded yet for initial screen
        // Use fallback if data.gameSettings isn't available yet
        const width = this.canvas.width || 960; // Default width
        const height = this.canvas.height || 540; // Default height
    ...
    checkCollisions(): void {
        ...
        this.enemies.forEach(enemy => {
            if (!enemy.markedForDeletion && !this.player!.markedForDeletion && this.isColliding(this.player!, enemy)) {
                ...
                // For simplicity, let's say contact with an enemy instantly destroys it and deals significant damage to player
                this.player!.takeDamage(enemy.health, this); // Enemy's health here represents contact damage
                ...
                this.playSound("explosion_sound"); // Changed to explosion_sound
            }
        });
    }
    ...
    renderTitleScreen(): void {
        if (!this.data) return;
        const titleImage = this.images.get("title_background"); // Assuming title_background image
        if (titleImage) {
