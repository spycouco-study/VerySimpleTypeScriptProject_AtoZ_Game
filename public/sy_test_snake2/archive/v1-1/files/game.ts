...
class AssetLoader {
    ...
    private assetData: AssetsConfig;

    constructor(assetData: AssetsConfig) {
        this.assetData = assetData;
    }

    async loadAll(): Promise<void> {
        const imagePromises = this.assetData.images.map(img => this.loadImage(img));
        const soundPromises = this.assetData.sounds.map(snd => this.loadSound(snd));
    ...
    private loadSound(asset: SoundAsset): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            // Use canplaythrough to ensure it's ready, or loadedmetadata for faster resolve
            audio.addEventListener('canplaythrough', () => {
                this.sounds.set(asset.name, audio);
                resolve();
            }, { once: true });
            audio.addEventListener('error', () => reject(`Failed to load sound: ${asset.path}`), { once: true });
    ...
    // For sound effects, create a new instance to allow overlapping playback
    playEffect(name: string): void {
        const originalSound = this.sounds.get(name);
        if (originalSound) {
            const effect = new Audio(originalSound.src);
            effect.volume = originalSound.volume;
            effect.play().catch(e => console.error(`Error playing sound effect ${name}:`, e));
        }
    }
    ...
    playBGM(name: string): void {
        ...
        if (bgm && bgm.paused) {
            ...
            bgm.play().catch(e => console.error(`Error playing BGM ${name}:`, e));
        }
    }
...
class Game {
    ...
    private gameTick(): void {
        ...
        // Self-collision (start checking from 4th segment to avoid immediate head-neck collision)
