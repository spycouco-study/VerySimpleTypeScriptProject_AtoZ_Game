interface GameSettings {
    ...
    backButtonLabel: string; // New: Label for the back button
    backButtonX: number;     // New: X position for the back button
    backButtonY: number;     // New: Y position for the back button
    backButtonWidth: number; // New: Width for the back button
    backButtonHeight: number; // New: Height for the back button
}
...
let isBackButtonHovered: boolean = false; // New: State for back button hover
let lastTypeSoundTime: number = 0;

let sceneHistory: string[] = []; // New: Stack to store visited scene IDs for back functionality
...
async function init() {
    ...
    canvas.addEventListener('click', () => {
        ...
        if (currentGameState === GameState.TITLE) {
            ...
            playSFX("click_sfx");
            sceneHistory = []; // Clear history on new game start
            resetSceneState();
        } else if (currentGameState === GameState.PRESENTATION) {
            // New: Handle back button click first
            if (isBackButtonHovered && sceneHistory.length > 0) {
                playSFX("click_sfx");
                const prevSceneId = sceneHistory.pop();
                const prevScene = gameData.scenes.find(s => s.id === prevSceneId);
                if (prevScene) {
                    currentSceneIndex = gameData.scenes.indexOf(prevScene);
                    resetSceneState();
                } else {
                    console.error("Previous scene not found in history, cannot go back.");
                }
                return; // Consume click, don't trigger other presentation logic
            }
            // If not back button, then proceed with normal presentation logic
            handlePresentationClickLogic();
        }
    });
...
// Renamed from handlePresentationClick to handlePresentationClickLogic
function handlePresentationClickLogic() {
    const currentScene = gameData.scenes[currentSceneIndex];

    // 1. Handle Choice Buttons (if any and hovered)
    if (currentScene.choices && hoveredButton) {
        playSFX("click_sfx");
        sceneHistory.push(currentScene.id); // Save current scene to history before moving
        ...
        if (targetScene) {
            ...
            console.error(`Target scene with ID ${hoveredButton!.target_scene_id} not found.`);
        }
    }

    // 2. Handle Text Progression
    ...
    if (displayedTextIndex < totalLines) {
        ...
        if (currentLineCharIndex < currentLine.length) {
            // Text is still typing, speed up to full line
            currentLineCharIndex = currentLine.length;
            playSFX("click_sfx");
            return; // Click handled (speed-up)
        }
    }

    // 3. Handle Next Line or Next Scene (if all text displayed and no choices or choices not clicked)
    // Only proceed if all text for the current line/scene is displayed.
    const lastLineContent = currentScene.text_content[totalLines - 1] || "";
    const allTextDisplayed = (displayedTextIndex >= totalLines - 1 && currentLineCharIndex >= lastLineContent.length) || totalLines === 0;

    if (allTextDisplayed && (!currentScene.choices || currentScene.choices.length === 0)) {
        if (displayedTextIndex < totalLines - 1) { // Move to next line in current scene
            displayedTextIndex++;
            currentLineCharIndex = 0;
            typingTimer = 0;
            playSFX("click_sfx");
            // No history push needed here, still in the same scene.
        } else { // All lines in current scene displayed, move to next scene
            if (currentScene.next_scene) {
                ...
                // No next_scene defined, transition to ending.
                currentGameState = GameState.ENDING;
                playSFX("click_sfx");
            }
        }
...
function resetSceneState() {
    ...
    typingTimer = 0;
    hoveredButton = null;
    isBackButtonHovered = false; // New: Reset back button hover state
    lastTypeSoundTime = 0;
}
...
function updatePresentation(deltaTime: number) {
    ...
    // Update hovered button for choices
    hoveredButton = null;
    ...
    // New: Update hovered state for back button
    isBackButtonHovered = false;
    if (sceneHistory.length > 0) { // Only if there's history to go back to
        const backBtnX = settings.backButtonX;
        const backBtnY = settings.backButtonY;
        const backBtnW = settings.backButtonWidth;
        const backBtnH = settings.backButtonHeight;
...
function renderPresentationScene() {
    ...
    const contentBoxHeight = canvas.height - 180 - 80; // Adjusted for padding / buttons
    const lineHeight = 30;
    ...
    // New: Render back button if history exists
    if (sceneHistory.length > 0) {
        renderButton(settings.backButtonLabel, settings.backButtonX, settings.backButtonY, settings.backButtonWidth, settings.backButtonHeight, isBackButtonHovered);
    }
