...
class Game {
    ...
    private songEndedNaturally: boolean = false; // Flag to track if BGM finished playing naturally
    ...
    private resetGame() {
        ...
        if (this.bgmAudio) {
            ...
            this.bgmAudio.onended = null; // Clear event listener
        }
        this.hitEffectParticles = [];
        this.songEndedNaturally = false; // Reset this flag for a new game
    }
    ...
    private startGame() {
        ...
        if (bgmAsset && bgmAsset.audio) {
            ...
            // Set up event listener for when the BGM finishes playing naturally
    ...
    private update(deltaTime: number) {
        if (this.gameState === GameState.PLAYING) {
            ...
            if (this.health <= 0) {
                ...
                if (this.bgmAudio) {
                    this.bgmAudio.pause();
                    this.bgmAudio.onended = null; // Prevent further trigger
                }
            } else if (this.songEndedNaturally && this.beatmapCurrentNoteIndex >= this.beatmap.notes.length && this.notes.length === 0) {
                // Game over if BGM has finished AND all notes have been processed AND no notes are left on screen
                this.gameState = GameState.GAME_OVER;
                if (this.bgmAudio) {
                    this.bgmAudio.pause();
                    this.bgmAudio.onended = null; // Prevent further trigger
    ...
    private drawPlayingScreen() {
        ...
        if (treeAsset) {
            // Place on bottom left, with some padding
            this.ctx.drawImage(
                treeAsset.img,
                30,
        ...
        if (santaAsset) {
            // Place on bottom right, with some padding
            this.ctx.drawImage(
                santaAsset.img,
