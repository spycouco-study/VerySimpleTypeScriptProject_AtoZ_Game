...
async function loadImage(imageAsset: ImageAsset): Promise<HTMLImageElement> {
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            // Added robustness: check if the loaded image has valid dimensions.
            // Some browsers might fire 'onload' even for invalid images, resulting in 0x0 dimensions.
            if (img.naturalWidth > 0 && img.naturalHeight > 0) {
                resolve(img);
            } else {
                reject(new Error(`Loaded image '${imageAsset.path}' has zero dimensions (potentially corrupted or failed to decode).`));
            }
        };
...
async function loadAssets(): Promise<void> {
    try {
        ...
        // Explicit check for the critical background image and player image after all assets are loaded
        const requiredImages = [
            gameData.gameSettings.backgroundImageName,
            gameData.player.imageName,
            gameData.bulletTypes[gameData.player.bulletImageName]?.imageName // Check player's bullet image
        ].filter(Boolean); // Filter out any potentially undefined bullet image names

        for (const imgName of requiredImages) {
            if (imgName && !assets.images[imgName]) {
                console.error(`CRITICAL: Image asset '${imgName}' not found in loaded assets.`);
                throw new Error(`Critical image asset '${imgName}' not loaded. Cannot proceed.`);
            }
        }
...
function draw() {
    ...
    // Draw solid background fallback (this is the very base layer, before any scrolling layers)
    ctx.fillStyle = "#333";
    ...
    if (backgroundImage) {
        if (backgroundImage.naturalWidth > 0 && backgroundImage.naturalHeight > 0) {
            // Image is valid and loaded, draw it
            ...
            // Image was found in assets, but its dimensions are zero or invalid.
            // This case should ideally be caught by loadImage's reject, but serves as a robust fallback.
            console.error(`ERROR: Background image '${gameData.gameSettings.backgroundImageName}' found in assets but has zero or invalid natural dimensions (${backgroundImage.naturalWidth}x${backgroundImage.naturalHeight}). Drawing static gray fallback.`);
            ctx.fillStyle = "#666"; // A different gray to indicate this specific error type (invalid image data)
            ctx.fillRect(0, 0, gameCanvas.width, gameCanvas.height);
        }
    } else {
        // Fallback for missing background image (not found in assets.images).
        // This block is executed if backgroundImage is not successfully loaded or asset name is wrong.
        console.warn(`WARNING: Background image '${gameData.gameSettings.backgroundImageName}' NOT found in loaded assets. Using scrolling gray fallback.`);
        ctx.fillStyle = "#555"; // A scrolling gray
