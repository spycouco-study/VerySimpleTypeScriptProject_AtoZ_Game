...
function update(deltaTime: number) {
    ...
    // Handle background scrolling independently
    ...
    if (backgroundImage && backgroundImage.naturalWidth > 0 && backgroundImage.naturalHeight > 0) {
        // Calculate scaled height to maintain aspect ratio while filling canvas width
        const aspectRatio = backgroundImage.naturalHeight / backgroundImage.naturalWidth;
        const drawHeight = gameCanvas.width * aspectRatio;
        
        backgroundY = (backgroundY + gameData.gameSettings.backgroundScrollSpeed * deltaTime);
        if (backgroundY >= drawHeight) {
            backgroundY %= drawHeight;
        }
    } else {
        // Fallback for backgroundY update if image is missing or invalid dimensions.
        // Revert to original behavior using canvasHeight if image is not loaded or has invalid dimensions.
...
function draw() {
    ...
    if (backgroundImage && backgroundImage.naturalWidth > 0 && backgroundImage.naturalHeight > 0) {
        ...
        ctx.drawImage(backgroundImage, 0, backgroundY, gameCanvas.width, drawHeight);
        ctx.drawImage(backgroundImage, 0, backgroundY - drawHeight, gameCanvas.width, drawHeight);
    } else {
        // Fallback for missing background image, use solid scrolling background
        console.warn(`Background image '${gameData.gameSettings.backgroundImageName}' not found in loaded assets or has invalid dimensions. Using fallback.`);
        ctx.fillStyle = "#555"; // A slightly different gray to distinguish the scrolling fallback
