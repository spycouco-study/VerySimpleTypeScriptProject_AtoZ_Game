interface GameSettings {
    ...
    backgroundScrollSpeed: number; // scrollSpeed를 backgroundScrollSpeed로 변경
    backgroundImageName: string; // 새로운 배경 이미지 에셋 이름
}
...
function loadLevel(levelIndex: number) {
    ...
    currentWaveIndex = 0;
    enemies = [];
    bullets = bullets.filter(b => b.shooter === "player"); // 적 총알만 지움
    isBossActive = false;
    bossInstance = null;
    enemiesToSpawnInCurrentWave = [];
}
    ...
    switch (currentGameState) {
        ...
        case GameState.MENU:
            if (keysPressed.has("Enter")) {
                currentGameState = GameState.CONTROLS; // 메뉴에서 ENTER 누르면 조작법 화면으로 이동
            }
            break;
        case GameState.CONTROLS: // 조작법 화면
            if (keysPressed.has("Enter")) {
                initializeNewGame(); // ENTER 누르면 새 게임 시작
        ...
        case GameState.GAME_OVER:
            if (keysPressed.has("Enter")) {
                currentGameState = GameState.CONTROLS; // 게임 오버 후 ENTER 누르면 조작법 화면으로 이동
            }
            break;
        case GameState.LEVEL_CLEAR:
            if (keysPressed.has("Enter")) {
                ...
                if (currentLevelIndex < gameData.levels.length) {
                    ...
                    currentGameState = GameState.MENU; // 모든 레벨 클리어 후 메뉴로 복귀
                    stopBGM();
                    playBGM("bgm_menu", gameData.assets.sounds.find(s => s.name === "bgm_menu")?.volume || 0.5, true);
                }
            }
...
function draw() {
    ...
    if (backgroundImage) {
        // 캔버스 크기에 맞춰 배경 이미지를 두 번 그려서 부드러운 스크롤 효과 생성
        ...
        // 배경 이미지가 없을 경우 기존의 단색 스크롤 배경을 fallback으로 사용
        ctx.fillStyle = "#555";
    ...
    switch (currentGameState) {
        ...
        case GameState.MENU:
            ...
            break;
        case GameState.CONTROLS: // 조작법 화면 그리기
