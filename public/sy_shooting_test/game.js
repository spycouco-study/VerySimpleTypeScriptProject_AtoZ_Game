...
class GameObject {
  constructor(pos, width, height, imageName = null, color = "#fff") {
    this.pos = pos;
    this.width = width;
    this.height = height;
...
class Player extends GameObject {
  constructor(pos, config, bulletConfig, gameSettings) {
  ...
  update(deltaTime, keysPressed, gameData) {
    let dx = 0;
    let dy = 0;
    ...
    this.pos.x = Math.max(
      this.width / 2,
      Math.min(gameData.gameSettings.canvasWidth - this.width / 2, this.pos.x)
    );
    this.pos.y = Math.max(
      this.height / 2,
      Math.min(gameData.gameSettings.canvasHeight - this.height / 2, this.pos.y)
    );
    ...
    if (
      !this.canShoot &&
      (performance.now() - this.lastShotTime) / 1000 >= 1 / this.fireRate
    ) {
      this.canShoot = true;
    }
  }
  shoot() {
    ...
    playSFX(
      "shoot",
      gameData.assets.sounds.find((s) => s.name === "shoot")?.volume || 0.5
    );
  }
  takeDamage(damage) {
    this.health -= damage;
    playSFX(
      "hit",
      gameData.assets.sounds.find((s) => s.name === "hit")?.volume || 0.4
    );
    if (this.health <= 0) {
      currentGameState = GameState.GAME_OVER;
      playSFX(
        "explosion",
        gameData.assets.sounds.find((s) => s.name === "explosion")?.volume ||
          0.7
      );
      spawnExplosion(
        this.pos.x,
        this.pos.y,
        gameData.gameSettings.explosionParticleCount
      );
    }
  }
}
class Bullet extends GameObject {
  ...
  constructor(pos, width, height, imageName, damage, velX, velY, shooter) {
...
class Enemy extends GameObject {
  constructor(pos, typeConfig, bulletConfig) {
  ...
  update(deltaTime, playerPos, gameData) {
    this.pos.y += this.speed * deltaTime;
    // Only allow shooting if on screen and has a bullet type and fire rate
    if (
      this.pos.y > 0 &&
      this.pos.y < gameData.gameSettings.canvasHeight &&
      this.bulletTypeConfig &&
      this.fireRate > 0
    ) {
  ...
  shoot(playerPos, customBulletSpeed = null) {
    if (!this.bulletTypeConfig) return;
    const bulletSpeed =
      customBulletSpeed !== null
        ? customBulletSpeed
        : this.bulletTypeConfig.speed;
    ...
    playSFX(
      "enemy_shoot",
      gameData.assets.sounds.find((s) => s.name === "enemy_shoot")?.volume ||
        0.3
    );
  }
  takeDamage(damage) {
...
class Boss extends Enemy {
  constructor(pos, bossConfig, bulletConfig) {
    super(
      pos,
      {
        ...bossConfig,
  ...
  update(deltaTime, playerPos, gameData) {
    ...
    const effectiveFireRate =
      this.bossConfig.fireRate * currentPhaseConfig.fireRateMultiplier;
    // Use enemyBulletSpeed from gameSettings if bulletTypeConfig is null, otherwise use its speed
    const baseBulletSpeed =
      this.bulletTypeConfig?.speed || gameData.gameSettings.enemyBulletSpeed;
    const effectiveBulletSpeed =
      baseBulletSpeed * currentPhaseConfig.bulletSpeedMultiplier;
    switch (currentPhaseConfig.movementPattern) {
      case "hover":
        if (this.pos.y < gameData.gameSettings.bossSpawnHeight) {
          ...
          this.pos.x +=
            Math.sin(this.phaseTimer * 0.5) * this.speed * deltaTime * 0.2;
        }
        break;
      case "charge":
        if (this.pos.y < gameData.gameSettings.bossSpawnHeight) {
          ...
          this.pos.x +=
            Math.sign(dx) * Math.min(Math.abs(dx), this.speed * deltaTime);
        }
        break;
      case "zigzag":
        if (this.pos.y < gameData.gameSettings.bossSpawnHeight) {
      ...
      case "sideways":
        if (this.pos.y < gameData.gameSettings.bossSpawnHeight) {
          ...
          this.pos.x +=
            Math.sin(this.phaseTimer * 4) * this.speed * deltaTime * 1.5;
        }
        break;
    }
    this.pos.x = Math.max(
    ...
    if (this.bulletTypeConfig && effectiveFireRate > 0) {
      if (
        (performance.now() - this.lastShotTime) / 1000 >=
        1 / effectiveFireRate
      ) {
    ...
    if (this.phaseTimer >= currentPhaseConfig.duration) {
      this.currentPhase =
        (this.currentPhase + 1) % this.bossConfig.phases.length;
      this.phaseTimer = 0;
    }
  }
  shoot(playerPos, bulletSpeed = null) {
    if (!this.bulletTypeConfig) return;
    const currentBulletSpeed =
      bulletSpeed !== null ? bulletSpeed : this.bulletTypeConfig.speed;
    const startX = this.pos.x;
...
class Particle extends GameObject {
  ...
  update(deltaTime) {
    ...
    this.color = `#${r.toString(16).padStart(2, "0")}${g
      .toString(16)
...
let enemiesToSpawnInCurrentWave = [];
...
async function loadAssets() {
  ...
  try {
    ...
    const imagePromises = gameData.assets.images.map(async (imgAsset) => {
      assets.images[imgAsset.name] = await loadImage(imgAsset);
    });
    const soundPromises = gameData.assets.sounds.map(async (sndAsset) => {
    ...
    playBGM(
      "bgm_menu",
      gameData.assets.sounds.find((s) => s.name === "bgm_menu")?.volume || 0.5,
      true
    );
  } catch (error) {
...
function playBGM(soundName, volume = 0.5, loop = true) {
  if (!audioContext || !assets.sounds[soundName] || currentBGM === soundName)
    return;
  stopBGM();
...
function startGame() {
  ...
  if (!playerBulletConfig) {
    console.error(
      "Player bullet type config not found:",
      playerConfig.bulletImageName
    );
    return;
  }
  player = new Player(
    new Vector2(
      ...
      gameData.gameSettings.canvasHeight - 50
    ),
    playerConfig,
    playerBulletConfig,
  ...
  playBGM(
    "bgm_game",
    gameData.assets.sounds.find((s) => s.name === "bgm_game")?.volume || 0.3,
    true
  );
}
...
function spawnEnemy(enemyType, x, y) {
  ...
  const enemyBulletConfig = enemyConfig.bulletType
    ? gameData.bulletTypes[enemyConfig.bulletType]
    : null;
  const enemy = new Enemy(new Vector2(x, y), enemyConfig, enemyBulletConfig);
  enemies.push(enemy);
}
function spawnBoss(bossType) {
  ...
  const bossBulletConfig = bossConfig.bulletType
    ? gameData.bulletTypes[bossConfig.bulletType]
    : null;
...
function spawnExplosion(x, y, count) {
  for (let i = 0; i < count; i++) {
    ...
    const life =
      Math.random() * gameData.gameSettings.explosionParticleLife + 0.1;
    particles.push(
      new Particle(new Vector2(x, y), size, life, vel, "#FFA500", "#FF0000")
    );
  }
}
...
function update(deltaTime) {
  if (!gameData) return;
  backgroundY =
    (backgroundY + gameData.gameSettings.scrollSpeed * deltaTime) %
  ...
  switch (currentGameState) {
    ...
    case GameState.PLAYING:
      if (player.health <= 0) {
        ...
        playBGM(
          "bgm_menu",
          gameData.assets.sounds.find((s) => s.name === "bgm_menu")?.volume ||
            0.5,
          true
        );
        return;
        ...
        if (!isBossActive) {
          if (enemiesToSpawnInCurrentWave.length > 0) {
            ...
            if (waveSpawnTimer >= currentSpawnGroup.spawnDelay) {
              const x =
                Math.random() * (gameData.gameSettings.canvasWidth - 60) + 30;
              const y = -50;
            ...
            while (
              currentWaveIndex < activeLevel.waves.length &&
              activeLevel.waves[currentWaveIndex].time <= levelTimer
            ) {
          ...
          const allWavesProcessed =
            currentWaveIndex >= activeLevel.waves.length &&
            enemiesToSpawnInCurrentWave.length === 0;
          const noRegularEnemiesOnScreen = !enemies.some((e) => !e.isBoss); // Check if any non-boss enemy exists
          if (
            !bossInstance &&
            levelTimer >= activeLevel.duration &&
            allWavesProcessed &&
            noRegularEnemiesOnScreen
          ) {
      ...
      for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        if (
          ...
          bullet.pos.y > gameData.gameSettings.canvasHeight + bullet.height ||
          bullet.pos.x < -bullet.width ||
          bullet.pos.x > gameData.gameSettings.canvasWidth + bullet.width
        ) {
          bullets.splice(i, 1);
        }
      }
      for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (
          !enemy.isBoss &&
          enemy.pos.y > gameData.gameSettings.canvasHeight + enemy.height / 2
        ) {
          enemies.splice(i, 1);
          continue;
        }
        ...
        if (player.isColliding(enemy)) {
          ...
          if (!enemy.isBoss) {
            // Regular enemies are destroyed on contact
            spawnExplosion(
              enemy.pos.x,
              enemy.pos.y,
              gameData.gameSettings.explosionParticleCount
            ...
            playSFX(
              "explosion",
              gameData.assets.sounds.find((s) => s.name === "explosion")
                ?.volume || 0.7
            );
            enemies.splice(i, 1);
            continue;
          ...
          if (bullet.shooter === "player" && bullet.isColliding(enemy)) {
            ...
            if (enemy.takeDamage(bullet.damage)) {
              ...
              if (enemy.isBoss) {
                ...
                playBGM(
                  "bgm_menu",
                  gameData.assets.sounds.find((s) => s.name === "bgm_menu")
                    ?.volume || 0.5,
                  true
                );
    ...
    case GameState.LEVEL_CLEAR:
      if (keysPressed.has("Enter")) {
        ...
        if (currentLevelIndex < gameData.levels.length) {
          ...
          playBGM(
            "bgm_game",
            gameData.assets.sounds.find((s) => s.name === "bgm_game")?.volume ||
              0.3,
            true
          );
        } else {
...
function draw() {
  ...
  ctx.fillStyle = "#555"; // Background starfield
  // Draw background twice to create seamless scrolling effect
  ctx.fillRect(0, backgroundY, gameCanvas.width, gameCanvas.height);
  ctx.fillRect(
    0,
    backgroundY - gameCanvas.height,
    gameCanvas.width,
    gameCanvas.height
  );
  switch (currentGameState) {
    ...
    case GameState.PLAYING:
      ...
      if (bossInstance) {
        ...
        ctx.fillStyle = "#F00";
        const healthRatio =
          bossInstance.health / bossInstance.bossConfig.health;
