    ...
    shoot(assets: GameAssets, game: Game): Bullet | null {
        if (this.fireCooldown <= 0) {
            ...
            return new Bullet(
                ...
                bulletConfig.height,
                bulletConfig.asset,
                0, // Player bullets have no X velocity
                this.bulletSpeed * -1, // Player bullets go up (negative Y velocity)
                this.bulletDamage,
                true // isPlayerBullet
            );
        }
...
class Bullet extends GameObject {
    ...
    constructor(x: number, y: number, width: number, height: number, assetName: string, velX: number, velY: number, damage: number, isPlayerBullet: boolean) {
    ...
    update(deltaTime: number, config: GameData): void {
        ...
        // Mark as dead if off-screen (consider X-axis too for aimed bullets)
        if (this.y + this.height < 0 || this.y > config.gameSettings.canvasHeight ||
            this.x + this.width < 0 || this.x > config.gameSettings.canvasWidth) {
            this.isAlive = false;
        }
    }
}
...
class Enemy extends GameObject {
    ...
    render(ctx: CanvasRenderingContext2D, assets: GameAssets): void {
        ...
        if (img) {
            ...
            // Move origin to the center of the enemy
            ctx.translate(this.x + this.width / 2, this.y + this.height / 2);
            // Rotate 180 degrees (Math.PI radians) to face downwards
            ctx.rotate(Math.PI);
            // Draw image, adjusting its position because the origin is now the center
    ...
    shoot(assets: GameAssets, game: Game, targetX: number, targetY: number): Bullet | null {
        if (this.fireRate > 0 && this.fireCooldown <= 0) {
            ...
            const bulletStartY = this.y + this.height; // Bullet starts at bottom of enemy

            // Calculate direction vector from bullet start to target
            const deltaX = targetX - bulletStartX;
            const deltaY = targetY - bulletStartY;
            const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);

            let velX = 0;
            let velY = this.bulletSpeed; // Default: straight down if target is same spot or very close

            if (distance > 0.01) { // Avoid division by zero or very small numbers
                velX = (deltaX / distance) * this.bulletSpeed;
                velY = (deltaY / distance) * this.bulletSpeed;
            } else {
                // If target is too close or same as bullet start, just fire straight down
                velX = 0;
    ...
    update(deltaTime: number): void {
        ...
        if (this.state === GameState.PLAYING) {
            ...
            this.enemies.forEach(enemy => {
                ...
                // Only attempt to shoot if enemy is alive, can shoot, and player exists/is alive
                if (enemy.isAlive && enemy.fireRate > 0 && this.player && this.player.isAlive) {
                    // Pass player's center coordinates as target for enemy aiming
