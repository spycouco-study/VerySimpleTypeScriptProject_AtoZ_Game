...
const canvas3 = document.getElementById("gameCanvas");
...
function drawBall() {
    ctx3.beginPath();
    ctx3.arc(x, y, ballRadius, 0, Math.PI * 2);
    ctx3.fillStyle = "#f60";
    ctx3.fill();
    ctx3.closePath();
}
...
function drawPaddle() {
    ...
    // **좌우 반전 로직**
    ctx3.save(); // 현재 캔버스 상태 저장
    if (leftPressed && !rightPressed) { // 왼쪽으로 움직일 때만 반전
        ctx3.translate(paddleX + paddleWidth, HEIGHT - paddleHeight); // 회전 중심을 이미지의 오른쪽 상단으로 이동
        ctx3.scale(-1, 1); // X축 반전 (좌우 반전)
        // 이미지를 그릴 때, x 좌표를 음수로 지정하여 반전된 상태에서 올바른 위치에 그려지도록 합니다.
        ctx3.drawImage(bearSprite, frame.sX, frame.sY, frame.sW, frame.sH, 0, 0, paddleWidth, paddleHeight // translate 했으므로 x,y는 0,0이 됩니다.
        );
    }
    else { // 오른쪽으로 움직이거나 멈춰 있을 때
        ctx3.drawImage(bearSprite, frame.sX, frame.sY, frame.sW, frame.sH, paddleX, HEIGHT - paddleHeight, paddleWidth, paddleHeight);
    }
    ctx3.restore(); // 캔버스 상태 복원 (다른 그리기 작업에 영향 주지 않도록)
}
// --- 스프라이트 애니메이션 관련 코드 끝 ---
...
function drawBricks() {
    ...
    for (let c = 0; c < BRICK_COLUMN_COUNT; c++) {
        for (let r = 0; r < BRICK_ROW_COUNT; r++) {
            if (bricks[c][r].status === 1) {
                ...
                bricks[c][r].y = brickY;
                ctx3.beginPath();
                ctx3.rect(brickX, brickY, BRICK_WIDTH, BRICK_HEIGHT);
                ctx3.fillStyle = bricks[c][r].color;
                ctx3.fill();
                ctx3.closePath();
            }
        }
    ...
    if (bossStatus === 1) {
        ...
        if (honeySprite.complete && honeySprite.naturalWidth !== 0) {
            ctx3.drawImage(honeySprite, BOSS_X, BOSS_Y, BOSS_SIZE, BOSS_SIZE);
        }
        else {
            // 이미지 로드 실패 시 대체 사각형 및 텍스트 표시
            ctx3.beginPath();
            ctx3.rect(BOSS_X, BOSS_Y, BOSS_SIZE, BOSS_SIZE);
            ctx3.fillStyle = "#ff5555"; // 보스 블록은 빨간색
            ctx3.shadowColor = "#ff0000";
            ctx3.shadowBlur = 10;
            ctx3.fill();
            ctx3.closePath();
            ctx3.shadowBlur = 0; // 그림자 효과 리셋
            ctx3.font = "bold 14px Arial";
            ctx3.fillStyle = "#fff";
            ctx3.textAlign = "center";
            ctx3.fillText("BOSS", BOSS_X + BOSS_SIZE / 2, BOSS_Y + BOSS_SIZE / 2 + 5);
...
function collisionDetection() {
    ...
    // 2. 보스 블록 충돌 감지 (승리 조건)
    if (bossStatus === 1) {
        if (x + ballRadius > BOSS_X && x - ballRadius < BOSS_X + BOSS_SIZE &&
...
function drawScore() {
    ...
    ctx3.fillStyle = "#333";
    ctx3.fillText("Score: " + score_, 8, 20);
}
function drawLives() {
    ctx3.font = "16px Arial";
    ctx3.fillStyle = "#333";
    ctx3.fillText("Lives: " + lives, WIDTH - 75, 20);
}
...
function imageLoaded() {
    ...
    if (imagesLoadedCount === totalImages) {
        // 모든 이미지가 로드되면 게임 시작
        // 배경 음악 초기화 및 재생 시도
        bgm3 = new Audio('bgm.mp3');
        bgm3.loop = true;
        bgm3.volume = 0.3; // 적절한 볼륨 설정
        bgm3.play().catch(error => {
        ...
        // **블록 깨짐 효과음 초기화**
        brickHitSound = new Audio('eat.mp3');
...
bearSprite.onerror = () => {
    //throw new Error("Failed to load bear.png");
...
function draw() {
    ...
    if (backImage.complete && backImage.naturalWidth !== 0) {
        ctx3.save(); // 현재 상태 저장 (globalAlpha를 변경하기 위함)
        ctx3.globalAlpha = 0.5; // 투명도 50%
        ctx3.drawImage(backImage, 0, 0, WIDTH, HEIGHT);
        ctx3.restore(); // 원래 상태 복원 (globalAlpha를 1.0으로 되돌림)
    }
    else {
        // 배경 이미지 로드 실패 시 투명 배경 유지 또는 단색 배경으로 대체
        ctx3.fillStyle = "#add8e6"; // 연한 파란색 배경
    ...
    if (bearSprite.complete && bearSprite.naturalWidth !== 0) {
        ...
        ctx3.beginPath();
        ctx3.rect(paddleX, HEIGHT - paddleHeight, paddleWidth, paddleHeight);
        ctx3.fillStyle = "#09f";
        ctx3.fill();
        ctx3.closePath();
    }
    ...
    collisionDetection();
    // Ball movement
    if (x + dx_ > WIDTH - ballRadius || x + dx_ < ballRadius) {
        dx_ = -dx_;
    }
    if (y + dy_ < ballRadius) {
        dy_ = -dy_;
