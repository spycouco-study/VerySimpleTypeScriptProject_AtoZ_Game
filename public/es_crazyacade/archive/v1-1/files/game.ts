...
class Game {
    ...
    private renderPlayingState(): void {
        ...
        this.powerUps.forEach(p => p.render(this.ctx, tileSize));
        this.bombs.forEach(b => b.render(this.ctx, tileSize));
        this.player.render(this.ctx, tileSize);
        this.enemies.forEach(e => e.render(this.ctx, tileSize));
        this.explosions.forEach(e => e.render(this.ctx, tileSize)); // Explosions on top of most things
    ...
    private resetGame(): void {
        ...
        this.playSound('bgm', true); // Start looping background music
...
class Player implements GameEntity {
    ...
    update(deltaTime: number, game: Game): void {
        ...
        // Convert tile-per-second speed to pixel-per-frame speed
        const movementSpeed = this.speed * settings.tileSize * deltaTime;

        let dx = 0;
        let dy = 0;

        // Determine desired movement direction from keyboard input
        ...
        const nextX = this.x + dx * movementSpeed;
        const nextY = this.y + dy * movementSpeed;

        // Attempt to move horizontally first, checking for collisions
        if (dx !== 0) {
            const testRectX = { x: nextX, y: this.y, width: this.width, height: this.height };
            if (!this.collidesWithWalls(testRectX, game)) {
                this.x = nextX;
            }
        }

        // Then attempt to move vertically, checking for collisions
        if (dy !== 0) {
            const testRectY = { x: this.x, y: nextY, width: this.width, height: this.height };
    ...
    placeBomb(game: Game): void {
        if (this.bombCount < this.maxBombCount) {
            ...
            const existingBomb = game.bombs.find(b =>
...
class Explosion implements GameEntity {
    ...
    render(ctx: CanvasRenderingContext2D, tileSize: number): void {
        ...
        const settings = gameInstance.getGameSettings();
