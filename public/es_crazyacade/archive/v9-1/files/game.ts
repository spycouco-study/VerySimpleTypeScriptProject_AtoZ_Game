...
interface GameData {
    gameSettings: {
        ...
        enemyDeathAnimationDuration: number; // New: Duration for enemy death animation
    };
    playerSettings: { // New settings for player movement behavior
        snapThreshold: number; // Max distance from tile center (as fraction of tileSize) to trigger snap
        snapStrengthFactor: number; // Fraction of player speed used for snapping
    };
    assets: {
        images: ImageAsset[];
        sounds: SoundAsset[];
...
interface GameEntity {
    ...
    isFinished?: boolean; // Optional: Indicates if an entity has completed its lifecycle and can be removed
}

// --- Game Class ---
class Game {
    private canvas: HTMLCanvasElement;
    private ctx: CanvasRenderingContext2D; // FIX: Changed CanvasContext2D to CanvasRenderingContext2D
    public gameData!: GameData; // Holds game settings, assets, and level data
    ...
    public tileMap: TileType[][] = []; // The current level's map
    ...
    private updatePlayingState(deltaTime: number): void {
        ...
        for (let i = this.enemies.length - 1; i >= 0; i--) {
            ...
            enemy.update(deltaTime, this); // Update enemy state (movement or dying animation)

            if (enemy.isFinished) { // Enemy has completed its death animation
                this.enemies.splice(i, 1);
                continue; // Skip further checks for this enemy as it's removed
            }

            // Only check collision with player if the enemy is NOT dying
            if (!enemy.isDying && this.player.isAlive && checkCollision(this.player, enemy)) {
        ...
        if (this.player.lives <= 0) {
            ...
            // Check if there are any *active* enemies left (not dying or finished)
            const activeEnemiesCount = this.enemies.filter(e => !e.isDying && !e.isFinished).length;
            if (activeEnemiesCount === 0) { // All active enemies defeated
    ...
    private renderPlayingState(): void {
        ...
        // Render active enemies first, then dying enemies on top (or reverse if desired)
        this.enemies.filter(e => !e.isDying).forEach(e => e.render(this.ctx, this));
        this.enemies.filter(e => e.isDying).forEach(e => e.render(this.ctx, this));
    ...
    public triggerExplosion(centerX: number, centerY: number, range: number): void {
        ...
        const processTile = (tx: number, ty: number, direction: 'up'|'down'|'left'|'right'|'center', endOfLine: boolean) => {
            ...
            for (let i = this.enemies.length - 1; i >= 0; i--) {
                const enemy = this.enemies[i];
                // Only hit enemies that are not already dying or finished
                if (!enemy.isDying && !enemy.isFinished && checkCollision(enemy, { x: pixelX, y: pixelY, width: tileSize, height: tileSize })) {
                    enemy.startDying(settings.enemyDeathAnimationDuration); // Start death animation
                    // We don't splice here, the enemy will be removed when isFinished is true in updatePlayingState
                }
            }
...
class Player implements GameEntity {
    ...
    resetForLevel(initialLives: number, maxBombCount: number, explosionRange: number, speed: number): void {
        this.lives = initialLives;
        this.bombCount = 0; // Reset active bombs
        this.maxBombCount = maxBombCount;
        this.explosionRange = explosionRange;
        this.speed = speed; // Restore original speed for level, this will be the current speed value
        this.isAlive = true;
        this.invincibleTimer = 0;
    }

    // Updates player's position, invincibility, and handles input.
    update(deltaTime: number, game: Game): void {
        ...
        const playerSettings = game.gameData.playerSettings; // Access player-specific settings
        const currentSpeedPerFrame = this.speed * settings.tileSize * deltaTime; // pixels per frame
        const tileSize = settings.tileSize;
        ...
        if (game.keyboardState.get('ArrowUp') || game.keyboardState.get('KeyW')) {
            inputDy = -1;
        } else if (game.keyboardState.get('ArrowDown') || game.keyboardState.get('KeyS')) {
            inputDy = 1;
        }
        if (game.keyboardState.get('ArrowLeft') || game.keyboardState.get('KeyA')) {
            inputDx = -1;
        } else if (game.keyboardState.get('ArrowRight') || game.keyboardState.get('KeyD')) {
            inputDx = 1;
        }

        // --- Calculate Base Movement based on Input ---
        // Normalize diagonal movement to prevent faster diagonal speed
        let baseMoveX = inputDx;
        let baseMoveY = inputDy;
        if (baseMoveX !== 0 && baseMoveY !== 0) {
            const magnitude = Math.sqrt(baseMoveX * baseMoveX + baseMoveY * baseMoveY);
            baseMoveX /= magnitude;
            baseMoveY /= magnitude;
        }

        // Apply speed to get pixel movement for base input
        baseMoveX *= currentSpeedPerFrame;
        baseMoveY *= currentSpeedPerFrame;

        // --- Snapping Logic for Grid Alignment ---
        const snapThreshold = playerSettings.snapThreshold * tileSize; // How far off center to trigger a snap
        const snapStrength = currentSpeedPerFrame * playerSettings.snapStrengthFactor; // Max speed for snapping towards center

        // Player's current center relative to its bounding box
        const playerBoxCenterX = this.x + this.width / 2;
        const playerBoxCenterY = this.y + this.height / 2;

        // Current grid cell the player's center is in
        const playerGridX = Math.floor(playerBoxCenterX / tileSize);
        const playerGridY = Math.floor(playerBoxCenterY / tileSize);

        // Target position if player were perfectly centered in their current grid cell, adjusted for player sprite size
        const targetCenteredX = playerGridX * tileSize + (tileSize - this.width) / 2;
        const targetCenteredY = playerGridY * tileSize + (tileSize - this.height) / 2;

        let finalMoveX = 0;
        let finalMoveY = 0;

        // Determine final X movement: prioritize input, otherwise snap
        if (inputDx !== 0) { // Player is actively trying to move horizontally
            finalMoveX = baseMoveX;
        } else { // No horizontal input, check for horizontal snapping
            const diffX = targetCenteredX - this.x;
            if (Math.abs(diffX) > tileSize * 0.01) { // Apply snap only if not already almost perfectly centered (to avoid jitter)
                finalMoveX = Math.sign(diffX) * Math.min(Math.abs(diffX), snapStrength);
            }
        }

        // Determine final Y movement: prioritize input, otherwise snap
        if (inputDy !== 0) { // Player is actively trying to move vertically
            finalMoveY = baseMoveY;
        } else { // No vertical input, check for vertical snapping
            const diffY = targetCenteredY - this.y;
            if (Math.abs(diffY) > tileSize * 0.01) {
                finalMoveY = Math.sign(diffY) * Math.min(Math.abs(diffY), snapStrength);
            }
        }

        // --- Collision Resolution (X then Y for sliding) ---
        let newX = this.x;
        let newY = this.y;

        // Attempt X movement and resolve tile collisions
        if (finalMoveX !== 0) {
            const testRectX = { x: this.x + finalMoveX, y: this.y, width: this.width, height: this.height };
            if (!this.collidesWithWalls(testRectX, game)) { // Only checks map tiles
                newX = this.x + finalMoveX;
            } else {
                // If collision, align player to the edge of the blocking tile
                if (finalMoveX > 0) { // Moving right
                    newX = Math.floor((this.x + this.width) / tileSize) * tileSize - this.width - 0.01;
                } else { // Moving left
                    newX = Math.ceil(this.x / tileSize) * tileSize + 0.01;
                }
            }
        }
        // Clamp X position to canvas boundaries after resolving tile collisions
        newX = Math.max(0, Math.min(newX, canvasWidth - this.width));


        // Attempt Y movement using the potentially updated X position and resolve tile collisions
        if (finalMoveY !== 0) {
            const testRectY = { x: newX, y: this.y + finalMoveY, width: this.width, height: this.height };
            if (!this.collidesWithWalls(testRectY, game)) { // Only checks map tiles
                newY = this.y + finalMoveY;
            } else {
                // If collision, align player to the edge of the blocking tile
                if (finalMoveY > 0) { // Moving down
                    newY = Math.floor((this.y + this.height) / tileSize) * tileSize - this.height - 0.01;
                } else { // Moving up
                    newY = Math.ceil(this.y / tileSize) * tileSize + 0.01;
                }
            }
        }
        // Clamp Y position to canvas boundaries after resolving tile collisions
        newY = Math.max(0, Math.min(newY, canvasHeight - this.height));
    ...
    // This method is for checking map tiles only, canvas boundary checks are handled separately in update().
    private collidesWithWalls(rect: { x: number, y: number, width: number, height: number }, game: Game): boolean {
        ...
        // Ensure iteration stays within the actual tileMap dimensions
        const mapWidthInTiles = game.tileMap[0] ? game.tileMap[0].length : 0;
        const mapHeightInTiles = game.tileMap.length;

        for (let y = Math.max(0, topTile); y <= Math.min(mapHeightInTiles - 1, bottomTile); y++) {
            for (let x = Math.max(0, leftTile); x <= Math.min(mapWidthInTiles - 1, rightTile); x++) {
...
class Enemy implements GameEntity {
    ...
    dyingTimer: number = 0;
    private initialDyingDuration: number = 0; // Stores the original duration when dying starts
    isFinished: boolean = false; // Added for game loop management
    ...
    // Initiates the enemy's dying animation sequence.
    startDying(duration: number): void {
        this.isDying = true;
        this.dyingTimer = duration;
        this.initialDyingDuration = duration;
        this.currentDirection = { dx: 0, dy: 0 }; // Stop movement when dying
        playSound('sfx_enemy_death'); // Play a death sound effect
    }
    ...
    update(deltaTime: number, game: Game): void {
        if (this.isFinished) return; // If already finished, no more updates

        if (this.isDying) {
            this.dyingTimer -= deltaTime;
            if (this.dyingTimer <= 0) {
                this.isFinished = true; // Mark as finished when animation is complete
            }
            return; // Skip movement logic if dying
        }
        ...
        // If the planned move doesn't cause collision with walls (map tiles or canvas boundaries) or other enemies, apply it.
    ...
    // Checks collision with solid map tiles AND canvas boundaries.
    private collidesWithWalls(rect: { x: number, y: number, width: number, height: number }, game: Game): boolean {
        const settings = game.getGameSettings();
        const tileSize = settings.tileSize;
        const canvasWidth = settings.canvasWidth;
        ...
        // 1. Check for collision with canvas boundaries
        if (rect.x < 0 || rect.x + rect.width > canvasWidth ||
            rect.y < 0 || rect.y + rect.height > canvasHeight) {
            return true; // Collision with canvas boundary
        }

        // 2. Check for collision with solid map tiles
        const leftTile = Math.floor(rect.x / tileSize);
        const rightTile = Math.floor((rect.x + rect.width - 0.01) / tileSize);
        const topTile = Math.floor((rect.y + rect.height - 0.01) / tileSize);
        const bottomTile = Math.floor((rect.y + rect.height - 0.01) / tileSize);
    ...
    private collidesWithOtherEnemies(rect: { x: number, y: number, width: number, height: number }, game: Game): boolean {
        for (const otherEnemy of game.getEnemies()) {
            // Only check collision with other enemies that are NOT this one and are NOT dying
            if (otherEnemy !== this && !otherEnemy.isDying && checkCollision(rect, otherEnemy)) {
                return true;
            }
        }
        return false;
    ...
    // Changed signature to receive game instance instead of tileSize directly
    render(ctx: CanvasRenderingContext2D, game: Game): void {
        if (this.isFinished) return; // Don't render if completely finished

        if (this.isDying) {
            // Calculate progress of the dying animation (0 at start, 1 at end)
            const progress = 1 - (this.dyingTimer / this.initialDyingDuration);
            const opacity = 1 - progress; // Fades out from 1 to 0
            ...
            const offsetX = (this.width - renderWidth) / 2; // Keep centered during scaling
            const offsetY = (this.height - renderHeight) / 2;
