...
class Game {
    ...
    private ctx: CanvasRenderingContext2D; // Corrected type: CanvasRenderingContext2D
...
class Player implements GameEntity {
    ...
    // New properties for allowing player to move off their own bomb
    private placedBombGridX: number | null = null;
    private placedBombGridY: number | null = null;
    ...
    resetForLevel(initialLives: number, maxBombCount: number, explosionRange: number, speed: number): void {
        ...
        this.placedBombGridX = null; // Reset bomb pass-through state
        this.placedBombGridY = null;
    }
    ...
    update(deltaTime: number, game: Game): void {
        ...
        // --- NEW: Reset bomb pass-through flag if player has moved off the bomb's tile ---
        if (this.placedBombGridX !== null && this.placedBombGridY !== null) {
            const playerCenterX = this.x + this.width / 2;
            const playerCenterY = this.y + this.height / 2;
            const currentPlayerGridX = Math.floor(playerCenterX / tileSize);
            const currentPlayerGridY = Math.floor(playerCenterY / tileSize);

            if (currentPlayerGridX !== this.placedBombGridX || currentPlayerGridY !== this.placedBombGridY) {
                // Player has moved off the tile where they placed the bomb
                this.placedBombGridX = null;
                this.placedBombGridY = null;
            }
        }
        // --- END NEW ---
        ...
        if (finalMoveX !== 0) {
            // Updated: Removed originalPlayerGridX,Y parameters as they are no longer needed
    ...
    // Updated signature: Removed originalPlayerGridX,Y parameters
    private getMaxAllowedMove(currentPos: number, targetPos: number,
        ...
        for (let dist = collisionStep; dist <= totalDistance; dist += collisionStep) {
            ...
            if (isXAxis) {
                ...
                // FIX: 'otherMovePos' was undefined. It should use 'otherAxisPos' which is passed as a parameter.
                testRect = { x: otherAxisPos, y: testMovePos, width: entityWidth, height: entityHeight };
            }

            // Updated: Removed originalPlayerGridX,Y parameters from collidesWithWalls call
    ...
    private collidesWithWalls(rect: { x: number, y: number, width: number, height: number }, game: Game): boolean {
        ...
        // NEW LOGIC: Player can pass through the bomb they just placed until they fully move off its tile.
        for (const bomb of game.getBombs()) {
            if (checkCollision(rect, bomb)) {
                ...
                // If this is the bomb the player just placed and is currently allowed to pass through
                if (this.placedBombGridX === bombGridX && this.placedBombGridY === bombGridY) {
                    continue; // Allow player to pass through this specific bomb
                }
                
                // Otherwise, it's a blocking bomb
    ...
    placeBomb(game: Game): void {
        if (this.bombCount < this.maxBombCount) {
            ...
            if (!existingBomb) {
                ...
                // NEW: Mark the grid position of the bomb just placed
                this.placedBombGridX = playerGridX;
                this.placedBombGridY = playerGridY;
            }
        }
    }
...
class Bomb implements GameEntity {
    ...
    constructor(x: number, y: number, width: number, height: number, delay: number, range: number, assetName: string, owner: Player) {
        ...
        this.exploded = false; // Ensure it's not exploded on creation
        this.owner = owner;
    }
