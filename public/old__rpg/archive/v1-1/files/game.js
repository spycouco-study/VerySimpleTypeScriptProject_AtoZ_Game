...
var GameState;
(function (GameState) {
    GameState[GameState["Village"] = 0] = "Village";
    GameState[GameState["Outside"] = 1] = "Outside";
    GameState[GameState["OutsideNorth"] = 2] = "OutsideNorth";
    GameState[GameState["OutsideWest"] = 3] = "OutsideWest";
    GameState[GameState["OutsideEast"] = 4] = "OutsideEast"; // New: Map to the east of Outside
})(GameState || (GameState = {}));
var ItemType;
(function (ItemType) {
...
class Player {
    constructor(x, y, image, playerData) {
        this.inventory = [];
        this.x = x;
        this.y = y;
        this.image = image;
    ...
    move(dx, dy, mapWidth, mapHeight, collidableTiles, enemies) {
        const newX = this.x + dx * this.speed;
        const newY = this.y + dy * this.speed;
        // Map boundary collision
        if (newX < 0 || newX + this.width > mapWidth)
            return;
        if (newY < 0 || newY + this.height > mapHeight)
            return;
...
class Enemy {
    constructor(x, y, image, enemyData) {
        this.isAlive = true;
    ...
    update(player, deltaTime, mapCollidableTiles, activeEnemies) {
        if (!this.isAlive)
            return;
...
class Item {
    constructor(x, y, type, image, itemTemplate, value) {
...
class Tile {
    constructor(x, y, width, height, image, collidable = false) {
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
...
class GameMap {
    constructor(assetManager, mapsGlobalData) {
        this.currentMapDataLayout = [];
        this.tiles = [];
        this.width = 0;
        this.height = 0;
        this.currentMapConfig = null; // Store current map config for easy access
...
class Game {
    constructor(canvasId) {
        this.enemies = [];
        this.items = [];
        this.currentGameState = GameState.Village;
        this.keysPressed = {};
        this.lastUpdateTime = 0;
        this.cameraX = 0;
        this.cameraY = 0;
        this.isGameOver = false; // New property for game over state
    ...
    async start() {
        ...
        await new Promise(resolve => this.assetManager.loadAll(resolve));
        ...
        this.player = new Player(initialVillagePos.xTile * tileWidth, initialVillagePos.yTile * tileHeight, this.assetManager.getImage('player'), this.gameData.playerData);
            ...
            if (entityData.type === "enemy" && entityData.enemyType) {
                ...
                if (itemTemplate) {
                    ...
                    switch (entityData.itemType) {
                        case 'potion':
                            itemEnum = ItemType.Potion;
                            break;
                        case 'sword':
                            itemEnum = ItemType.Sword;
                            break;
                        default:
                            console.warn(`Unknown item type: ${entityData.itemType}`);
                            continue;
                    }
        ...
        for (const transition of currentMapConfig.transitions) { // Iterate through all defined transitions
            ...
            if (this.player.checkCollision(playerRect, triggerRect)) {
                ...
                switch (transition.targetState) { // Use transition's targetState
                    case 'Village':
                        nextState = GameState.Village;
                        break;
                    case 'Outside':
                        nextState = GameState.Outside;
                        break;
                    case 'OutsideNorth':
                        nextState = GameState.OutsideNorth;
                        break; // New
                    case 'OutsideWest':
                        nextState = GameState.OutsideWest;
                        break; // New
                    case 'OutsideEast':
                        nextState = GameState.OutsideEast;
                        break; // New
                    default:
    ...
    handlePlayerAttack() {
        for (const enemy of this.enemies) {
            if (enemy.isAlive) {
                const distance = Math.sqrt(Math.pow(this.player.x - enemy.x, 2) +
    ...
    restartGame() {
        ...
        const tileWidth = this.gameData.mapsData.tileWidth;
        const tileHeight = this.gameData.mapsData.tileHeight;
        this.player = new Player(initialVillagePos.xTile * tileWidth, initialVillagePos.yTile * tileHeight, this.assetManager.getImage('player'), this.gameData.playerData // Re-initialize with original player stats
        );
