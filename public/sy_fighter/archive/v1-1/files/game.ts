...
class Character {
    ...
    changeState(newState: CharacterState) {
        ...
        switch (newState) {
            ...
            case CharacterState.ATTACKING:
                // Animation name set by attack() method
                this.attackTimer = this.gameSettings.attackDuration; // Generic attack duration
                break;
    ...
    attack(attackAnimName: string) {
        if (this.characterState === CharacterState.ATTACKING || this.characterState === CharacterState.JUMPING || this.characterState === CharacterState.HIT) {
            return; // Cannot attack while already attacking, jumping or hit
        }
        ...
        this.currentAnimationName = attackAnimName;
...
// Polyfill for requestAnimationFrame for older browsers if needed
