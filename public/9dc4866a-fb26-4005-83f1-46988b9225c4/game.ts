...
class Game {
    ...
    private gridOffsetX: number = 0; // New: X offset to center the grid
    private gridOffsetY: number = 0; // New: Y offset to center the grid
    ...
    constructor(canvasId: string) {
        ...
        this.loadConfig().then(() => {
            ...
            this.loadAssets().then(() => {
                this.calculateGridPosition(); // New: Calculate grid position after canvas size is set
                this.initGame();
    ...
    private handleMouseDown(event: MouseEvent): void {
        ...
        switch (this.gameState) {
            ...
            case GameState.PLAYING:
                ...
                // Adjust mouse coordinates by grid offset
    ...
    private drawGridAndBlocks(): void {
        const { rows, cols, blockSize } = this.config.grid;
        const offsetX = this.gridOffsetX; // Use stored offset
        const offsetY = this.gridOffsetY; // Use stored offset
        
        // Draw grid background and blocks
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                const blockType = this.grid[r][c];
                const x = c * blockSize + offsetX; // Apply offset
                const y = r * blockSize + offsetY; // Apply offset
